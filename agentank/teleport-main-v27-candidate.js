...
function onIdle(me, enemy, game) {
  ...
  function tryImmediateLaneEscape() {
    ...
    if (bulletThreatAt(nx, ny, 4) || overloadThreatAt(nx, ny)) return false;
    me.go();
    return true;
  }
  ...
  function grassAmbushMap(star) {
    ...
    for (var x = 1; x < w - 1; x++) {
      for (var y = 1; y < h - 1; y++) {
        ...
        grass++;
        if (dist(x, y, star[0], star[1]) <= 6) nearStar++;
      }
    }
  ...
  function bestGrassAmbush(star, myDist, enemyDist) {
    ...
    for (var x = 1; x < w - 1; x++) {
      for (var y = 1; y < h - 1; y++) {
        ...
        if (starGap < 1 || starGap > 6) continue;
  ...
  function bestGrassControlAnchor(star) {
    if (!star || !enemyTank || !teleportReady()) return null;
    var enemyRoute = predictRoute([ex, ey], star, 7);
    var best = null, bestScore = -9999;
    for (var x = 1; x < w - 1; x++) {
      for (var y = 1; y < h - 1; y++) {
        if (tile(x, y) !== "o") continue;
        ...
        var starGap = dist(x, y, star[0], star[1]);
        if (starPath > 7 && starGap > 7) continue;
        var controlsRoute = canShoot([x, y], star);
        for (var i = 0; i < enemyRoute.length; i++) {
          if (canShoot([x, y], enemyRoute[i])) controlsRoute = true;
        }
        if (!controlsRoute && starPath > 5) continue;
        var ed = dist(x, y, ex, ey);
        if (ed <= 4) continue;
        var score = 230 - Math.min(starPath, starGap) * 24 - dist(px, py, x, y) * 2;
        score += openNeighborCount(x, y) * 10;
        score += Math.min(ed, 8) * 3;
        if (canShoot([x, y], star)) score += 60;
        for (var r = 0; r < enemyRoute.length; r++) {
          if (canShoot([x, y], enemyRoute[r])) score += 42 - r * 4;
          if (dist(x, y, enemyRoute[r][0], enemyRoute[r][1]) <= 2) score += 8;
        }
        if ((x === ex || y === ey) && ed <= 9) score -= 80;
        if (score > bestScore) {
          bestScore = score;
          best = [x, y];
        }
  ...
  function tryGrassAmbushFire(star, enemyDist) {
    ...
    if (dist(px, py, star[0], star[1]) > 7) return false;
    if (me.bullet || me.status.fireLocked) return false;
  ...
  function tryGrassAmbushSetup(star, myDist, enemyDist) {
    ...
    var target = bestGrassAmbush(star, myDist, enemyDist) || bestGrassControlAnchor(star);
  ...
  if (game.star && frame < 10 && tryQuietStarRush(game.star)) return;
  ...
  if (game.star && frame >= 30) {
    var earlyStar = game.star;
    var earlyMyPath = pathInfo(myPos, earlyStar, true) || pathInfo(myPos, earlyStar, false);
    var earlyMyDist = earlyMyPath ? earlyMyPath.dist : 999;
    var earlyEnemyDist = enemyTank ? pathDist([ex, ey], earlyStar) : 999;
    if (tryGrassAmbushSetup(earlyStar, earlyMyDist, earlyEnemyDist)) return;
  }
