...
function onIdle(me, enemy, game) {
  ...
  function protectingLead() {
    return frame >= 35 && starLead() >= 2;
  }
  ...
  function directLaneTrapAt(x, y, maxCells) {
  ...
  function leadHiddenLaneAt(x, y) {
    if (_lastEX < 0 || _lastESkill !== "cloak") return false;
    var memory = grassMaze() ? 38 : 22;
    if (frame - _lastSeen > memory) return false;
    var gap = dist(_lastEX, _lastEY, x, y);
    if (gap > (grassMaze() ? 14 : 10)) return false;
    return x === _lastEX || y === _lastEY;
  }

  function leadDangerAt(x, y) {
    if (!protectingLead()) return false;
    if (bulletThreatAt(x, y, 10)) return true;
    if (directLaneTrapAt(x, y, 10)) return true;
    if (oneStepAimAt(x, y)) return true;
    if (movingEnemyFireSetupAt(x, y)) return true;
    if (hiddenLaneAt(x, y) || leadHiddenLaneAt(x, y)) return true;
  ...
  function safeCell(x, y, strict) {
    ...
    if (strict && longLaneAimAt(x, y)) return false;
    if (strict && oneStepAimAt(x, y)) return false;
    if (strict && movingEnemyFireSetupAt(x, y)) return false;
    if (strict && hiddenLaneAt(x, y)) return false;
    if (strict && rememberedSpawnThreatAt(x, y)) return false;
    return true;
  }
  ...
  function fatalStepAt(x, y) {
    ...
    if (!farStarPickup && longLaneAimAt(x, y)) return true;
    if (oneStepAimAt(x, y)) return true;
    if (movingEnemyFireSetupAt(x, y)) return true;
    if (overloadThreatAt(x, y)) return true;
    return false;
  ...
  function pickupLaneTrapAt(x, y) {
    if (bulletThreatAt(x, y, 8)) return true;
    if (directLaneTrapAt(x, y, 10)) return true;
    if (oneStepAimAt(x, y)) return true;
    if (movingEnemyFireSetupAt(x, y)) return true;
    if (leadHiddenLaneAt(x, y)) return true;
    return false;
  }

  function tryAdjacentStarPickup(star) {
    if (!star || dist(px, py, star[0], star[1]) !== 1) return false;
    if (starsOf(me) + 1 - starsOf(enemy) >= 2 && pickupLaneTrapAt(star[0], star[1])) return false;
  ...
  function starRaceNeedsFastRoute(strictDist, fastDist, enemyDist) {
    if (starLead() >= 2) return false;
    if (fastDist >= strictDist || fastDist >= 999 || enemyDist >= 999) return false;
    if (strictDist <= enemyDist + 1) return false;
    if (fastDist > enemyDist + 1) return false;
    return enemySkillIs("shield") || enemySkillIs("boost") || enemySkillIs("teleport") || enemySkillIs("cloak") || enemyDist <= strictDist;
  }
  ...
  function leadEscapeDir() {
    var best = null, bestScore = -9999;
    for (var i = 0; i < 4; i++) {
      var d = dirs[i];
      var n = add(myPos, delta(d));
      if (!safeCell(n[0], n[1], true)) continue;
      if (leadDangerAt(n[0], n[1])) continue;
      if (bulletActionTrap(d)) continue;
      var score = 80 - turnCost(dir, d) * 10;
      if (enemyTank) {
        score += dist(n[0], n[1], ex, ey) * 3;
        if (n[0] !== ex && n[1] !== ey) score += 12;
      }
      if (game.star) score -= Math.max(0, dist(n[0], n[1], game.star[0], game.star[1]) - 3);
      if (d === dir) score += 8;
      if (score > bestScore) { bestScore = score; best = d; }
    }
    return best;
  ...
  function tryProtectLead() {
    if (!protectingLead()) return false;
    var f = add(myPos, delta(dir));
    if (!leadDangerAt(px, py) && !leadDangerAt(f[0], f[1]) && !bulletActionTrap(dir)) return false;
    ...
    var escape = leadEscapeDir() || escapeAimDir();
    if (escape && moveDir(escape)) return true;
    return false;
  }

  function urgentAimEscapeDir() {
    if (!aimDangerHere() && !oneStepAimAt(px, py)) return null;
  ...
  function tryPreemptAimDodge() {
    var aimDanger = aimDangerHere() || oneStepAimAt(px, py);
  ...
  function tryStarRacePressure(myDist, enemyDist) {
    ...
    if (bulletThreatAt(px, py, 4) || overloadThreatAt(px, py) || hiddenLaneAt(px, py)) return false;
  ...
  if (game.star) {
    ...
    if (urgent && starRaceNeedsFastRoute(myDist, fastDist, enemyDist) && moveToward(star, false)) return;
