...
function onIdle(me, enemy, game) {
  ...
  function hiddenEnemyLaneAt(tx, ty, strict) {
    if (enemy && enemy.tank) return false;
    if (_lastEX < 0 || frame - _lastEnemySeenFrame > (strict ? 18 : 10)) return false;
    var gap = dist(_lastEX, _lastEY, tx, ty);
    ...
    return strict && gap <= 5 && (tx === _lastEX || ty === _lastEY);
  }
  ...
  function safeCell(tx, ty, strict) {
    ...
    if (strict && hiddenEnemyLaneAt(tx, ty, true)) return false;
  ...
  function dangerousLeadSkill() {
    return enemySkillIs("poison") || enemySkillIs("cloak") ||
      ...
      enemySkillIs("freeze") || enemySkillIs("teleport");
  }
  ...
  function tryPoisonTempo() {
    ...
    var earlyFarOverload = enemySkillIs("overload") && frame < 18 && d2 > 18;
    if (game.star) {
    ...
    if (enemySkillIs("overload") && (enemyOverloadArmed() || enemySkillReady("overload", 4))) {
      ...
      if (game.star) {
        ...
        if (earlyFarOverload) starContest = false;
      }
    ...
    if (game.star) {
      ...
      if (earlyFarOverload) contest = false;
  ...
  function tryFreezeTempo() {
    if (!freezeReady() || !enemy || !enemy.tank || enemyDebuffed()) return false;
    var d2 = dist(px, py, ex, ey);

    if (game.star) {
      var sx = game.star[0], sy = game.star[1];
      var mySd = bfsDist(px, py, sx, sy);
      var eSd = bfsDist(ex, ey, sx, sy);
      var eRace = eSd;
      var boostRace = enemySkillIs("boost") && (enemySkillReady("boost", 8) ||
          (enemy.status && enemy.status.boosted) ||
          (enemy.skill && enemy.skill.activeType === "boost"));
      if (boostRace) eRace = Math.ceil(eSd / 2);

      var closeRace = mySd <= 8 && eRace <= mySd + 1;
      var decisiveRace = mySd <= 3 || eRace <= 3 || starsOf(me) <= starsOf(enemy) + 1;
      if (closeRace && (decisiveRace || boostRace || enemySkillIs("teleport") || enemySkillIs("shield"))) {
        taunt("冰封星路，队长先到。");
        me.freeze(); return true;
      }
    }

    if (enemySkillIs("overload") && (enemyOverloadArmed() || enemySkillReady("overload", 2))) {
      if (d2 <= 8 || enemyGunLineAt(px, py, true) || overloadReturnRisk(px, py) ||
          enemyOverloadLaneAt(px, py)) {
        taunt("双发炮口，先冻住。");
        me.freeze(); return true;
      }
    }

    if (enemyAimedAtUs() && d2 <= 7 && !aheadOnStars()) {
      taunt("枪线太直，冻结。");
      me.freeze(); return true;
    }
  ...
  function wallPressure(tx, ty) {
    var n = 0;
    for (var i = 0; i < 4; i++) {
      if (!ok(tx + ddx[i], ty + ddy[i])) n++;
    }
    return n;
  }

  function overloadEdgePressure(tx, ty) {
    return tx <= 1 || tx >= 17 || ty <= 1 || ty >= 13 ||
      wallPressure(tx, ty) >= 2 || enemyOverloadLaneAt(tx, ty);
  }

  function tryOverloadEdgeEscape() {
    if (!enemy || !enemy.tank || !enemySkillIs("overload")) return false;
    if (starsOf(me) < starsOf(enemy) + 2) return false;
    if (!enemyOverloadArmed() && !enemySkillReady("overload", 2)) return false;
    if (game.star && dist(px, py, game.star[0], game.star[1]) <= 1) return false;
    if (dist(px, py, ex, ey) > 8 && !enemyOverloadLaneAt(px, py)) return false;
    if (!overloadEdgePressure(px, py)) return false;
    ...
    for (var i = 0; i < 4; i++) {
      ...
      if (!safeCell(nx, ny, true)) continue;
      var score = 0;
      if (nx > 1 && nx < 17 && ny > 1 && ny < 13) score += 18;
      if (!enemyOverloadLaneAt(nx, ny)) score += 10;
      score -= wallPressure(nx, ny) * 5;
      score -= dist(nx, ny, 9, 7);
      score += Math.min(dist(nx, ny, ex, ey), 7);
      if (d === dir) score += 2;
      if (score > bestScore) { bestScore = score; best = d; }
    }
    if (best) {
  ...
  function useSkill() {
    ...
    if (type === "freeze") {
      if (enemy && enemy.tank && !enemyDebuffed()) {
        var fd = dist(px, py, ex, ey);
        if (enemyAimedAtUs() && fd <= 7) { me.freeze(); return true; }
        if (fd <= 4 && (los(dir, ex, ey) || enemyQuickAimAt(px, py) || game.star)) {
          me.freeze(); return true;
        }
      }
    }
  ...
  function tryFreezeCombo() {
    if (!freezeReady() || !enemy || !enemy.tank || enemyDebuffed()) return false;
    if (enemyShotBlocked() || me.bullet) return false;
    var d2 = dist(px, py, ex, ey);

    if (d2 <= 3 && los(dir, ex, ey) && forwardOpen(dir) &&
        !bulletThreatAt(px, py, 4) && !enemyGunLineAt(px, py, true)) {
      taunt("冻结两拍，炮口正中。");
      me.freeze();
      me.fire();
      _lastFireFrame = frame + 1;
      return true;
    }
  ...
  function tryPickupStar() {
    ...
    if (d) {
      ...
      if (mySd <= 1 && dd) {
        var nx = px + dd[0], ny = py + dd[1];
        if (ok(nx, ny) && !(nx === ex && ny === ey) &&
            !bulletThreatAt(nx, ny, 3) && !enemyGunLineAt(nx, ny, true) &&
            !(enemySkillIs("overload") && enemyOverloadArmed() && enemyOverloadLaneAt(nx, ny))) {
          goDir(d); return true;
        }
      }
    }
  ...
  // 2b. With a clear lead, do not sit on overload's near-wall double-shot lane.
  if (tryOverloadEdgeEscape()) return;
  ...
  // 5. Use skills as tempo weapons against global skills and star races.
  if (tryFreezeTempo()) return;
  ...
  // 7. Freeze/poison combo (strategic use before engaging)
  if (tryFreezeCombo()) return;
