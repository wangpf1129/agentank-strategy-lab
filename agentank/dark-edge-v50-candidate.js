...
function onIdle(me, enemy, game) {
  ...
  function edgeTrapCell(tx, ty) {
    if (tx <= 1 || tx >= 17 || ty <= 1 || ty >= 13) return true;
    var blocked = 0;
    if (!ok(tx + 1, ty)) blocked++;
    if (!ok(tx - 1, ty)) blocked++;
    if (!ok(tx, ty + 1)) blocked++;
    if (!ok(tx, ty - 1)) blocked++;
    return blocked >= 2;
  }
  ...
  function tryPoisonTempo() {
    ...
    if (enemySkillIs("overload") && (enemyOverloadArmed() || enemySkillReady("overload", 4))) {
      ...
      if (game.star) {
        ...
        if (frame < 18 && d2 > 12) starContest = false;
      }
  ...
  function enemyOverloadLaneAt(tx, ty) {
    ...
    if (Math.abs(ty - ey) <= 1 && tx !== ex) {
      ...
      if (turnCost(eDir, hAim) <= (edgeTrapCell(tx, ty) ? 2 : 1) &&
          ok(ex, ty) && losFrom(ex, ty, hAim, tx, ty)) return true;
    }
    if (Math.abs(tx - ex) <= 1 && ty !== ey) {
      var vAim = ty < ey ? "up" : "down";
      if (turnCost(eDir, vAim) <= (edgeTrapCell(tx, ty) ? 2 : 1) &&
          ok(tx, ey) && losFrom(tx, ey, vAim, tx, ty)) return true;
    }
    return false;
  }
  ...
  function tryLeadKite() {
    ...
    for (var i = 0; i < 4; i++) {
      ...
      if (edgeTrapCell(nx, ny)) score -= enemySkillIs("overload") ? 12 : 5;
      else score += 4;
      if (d === dir) score += 2;
      if (score > bestScore) { bestScore = score; best = d; }
    }
  ...
  function tryControlPosition() {
    if (!enemy || !enemy.tank) return false;
    if (starsOf(me) < starsOf(enemy) + 2) return false;
    if (!(enemySkillIs("overload") || enemySkillIs("boost") || enemySkillIs("freeze"))) return false;
    if (game.star && dist(px, py, game.star[0], game.star[1]) <= 5) return false;
    if (!edgeTrapCell(px, py) && dist(px, py, 9, 7) <= 3) return false;

    var points = [[9,7],[8,7],[10,7],[9,6],[9,8],[8,6],[10,8],[7,7],[11,7]];
    var best = null, bestScore = -9999;
    for (var i = 0; i < points.length; i++) {
      var tx = points[i][0], ty = points[i][1];
      if (!ok(tx, ty)) continue;
      var d = bfsDir(tx, ty);
      if (!d) continue;
      var dd = dv[d], nx = px + dd[0], ny = py + dd[1];
      if (!safeCell(nx, ny, true)) continue;
      var score = -bfsDist(px, py, tx, ty) * 2 + dist(tx, ty, ex, ey);
      if (!edgeTrapCell(tx, ty)) score += 8;
      if (d === dir) score += 2;
      if (score > bestScore) { bestScore = score; best = d; }
    }
    if (best) { goDir(best); return true; }
  ...
  // 3c. With a solid lead, hold center control instead of chasing into edges.
  if (tryControlPosition()) return;
