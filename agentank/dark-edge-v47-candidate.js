...
function onIdle(me, enemy, game) {
  ...
  function dangerousLeadSkill() {
    return enemySkillIs("poison") || enemySkillIs("cloak") ||
      ...
      enemySkillIs("boost") || enemySkillIs("freeze");
  }
  ...
  function tryPoisonTempo() {
    ...
    if (game.star) {
      ...
      var contest = eSd <= mySd + 2 && mySd <= 14;
      if (contest && (enemyTempoSkillReady(4) || eSd < mySd || enemySkillIs("shield"))) {
        taunt("星路封毒，节奏归我。");
        me.poison(); return true;
      }
    }
  ...
  function tryLeadKite() {
    ...
    if (!aheadOnStars()) return false;
    var closeEnemy = dist(px, py, ex, ey) <= 4;
    if (game.star) {
      var myStar = dist(px, py, game.star[0], game.star[1]);
      var enemyStar = dist(ex, ey, game.star[0], game.star[1]);
      if (!closeEnemy && myStar <= enemyStar) return false;
      if (closeEnemy && myStar <= 2 && myStar <= enemyStar) return false;
    }
