var _lastX = -1, _lastY = -1, _stuck = 0, _postTeleportUntil = -1;
...
function onIdle(me, enemy, game) {
  ...
  function castTeleport(x, y) {
    _postTeleportUntil = frame + 10;
    me.teleport(x, y);
  }

  function afterTeleportWindow() {
    return frame <= _postTeleportUntil;
  }
  ...
  function bulletTurnsTo(x, y) {
    if (!enemy || !enemy.bullet) return 999;
    var bp = enemy.bullet.position;
    var bd = enemy.bullet.direction;
    if (!bd || !dv[bd]) return 999;
    if (!laneThreatFrom(bp[0], bp[1], bd, x, y, 40)) return 999;
    return Math.ceil(dist(bp[0], bp[1], x, y) / 2);
  }
  ...
  function postTeleportLaneTrapAt(x, y) {
    if (!enemyTank || enemyDebuffed()) return false;
    if (x !== ex && y !== ey) return false;
    var ed = dist(x, y, ex, ey);
    if (ed > 12) return false;
    var need = x === ex ? (y < ey ? "up" : "down") : (x < ex ? "left" : "right");
    return turnCost(eDir, need) <= 1 && losFrom(ex, ey, need, x, y);
  }
  ...
  function tryDodge() {
    if (
      !bulletThreatAt(px, py, 10) &&
      !overloadThreatAt(px, py) &&
      !(afterTeleportWindow() && postTeleportLaneTrapAt(px, py)) &&
      !hiddenLaneAt(px, py)
    ) return false;
    var best = null, bestScore = -9999;
    for (var i = 0; i < 4; i++) {
  ...
  function aimDangerHere() {
    if (!enemyTank || dist(px, py, ex, ey) > 6) return false;
    if (enemy && enemy.bullet) return false;
  ...
  function tryAttack(urgentStar) {
    ...
    if (!urgentStar && canShoot(myPos, [ex, ey])) {
      ...
      if (dir === want && bulletTurnsTo(px, py) <= Math.ceil(dist(px, py, ex, ey) / 2)) return false;
      if (dir === want) me.fire();
      else turnTo(want);
      return true;
    }
  ...
  if (game.star) {
    ...
    if (shouldTeleport) {
      ...
      if (landing) {
        castTeleport(landing[0], landing[1]);
        return;
      }
    }
