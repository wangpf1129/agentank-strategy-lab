...
function usage() {
  return [
    ...
    "  --output-dir <path>         Where replay JSON files are saved. Default: agentank/lab/data/matches",
    "  --run-dir <path>            Where run logs are saved. Default: agentank/lab/data/challenge-runs",
    ...
    "  AGENTANK_FREEZE_KEY=<key> AGENTANK_TELEPORT_KEY=<key> node agentank/lab/scripts/run-real-challenges.mjs --opponents 829 --maps arena --repeat 5 --execute",
    "",
...
const outputDir = readOption(argv, "--output-dir", "agentank/lab/data/matches");
const runDir = readOption(argv, "--run-dir", "agentank/lab/data/challenge-runs");
const run = {
  runId: safeTimestamp(),
