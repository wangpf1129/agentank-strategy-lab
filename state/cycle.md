...
Goal: shield-main / 500 / boost stable 2600. Axis: v70 is published after Champion top-50 sample: visible bullets count as active danger and urgent bullet-lane exits outrank future reply-line caution.

## Last Command Results

- Shield-main / tank 4839 `500` v70 Champion top-50 visible-bullet escape publish:
  - User reported `500` had reached Master and requested 10-20 real fights against top-50 Champion tanks, with both wins and losses reviewed before iteration.
  - Pre-run gates on live v69 passed: `npm run check`; focused shield-main tests 28/28; `.env npm run simulate` 9/9 at `/tmp/agentank-runs/simulations/2026-07-09T11-29-25-305Z`; `npm run challenge:dry` at `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-09T11-29-53-773Z.json`. Public score leaderboard snapshot `/tmp/agentank-runs/leaderboards-public-score-top50/2026-07-09T11-30-09-045Z.json` showed the top 50 were all Champion I, score 2254-2667.
  - User-requested real sample used explicit public top-50 ids, one match per opponent, random map, limit 15, drawdown stop 80: `/tmp/agentank-runs/shield-main/real15-v69-champion-top50/challenge-runs/2026-07-09T11-30-33-764Z.json`.
  - Real15 result: 4W/11L against the first 15 Champion I targets, run score 1547 -> 1564, peak 1584, no errors. Wins: `mat_Asrg2cHUpCu2ZMKjG`, `mat_Hw3bgLmypSaKC8Xkv`, `mat_9NyJbjKtvzpGcrukt`, `mat_E1bXTfoI2io0dz3fn`; losses: `mat_Iqig0VkVSry34Tlgw`, `mat_AkRsLSSIKLW5yJR9f`, `mat_0l73bU3qMJdJRLRNU`, `mat_2ATPnIZnJeKHcRm6q`, `mat_KajQitdNA4ACnO9j6`, `mat_AeymrJcSnOGHrsUDT`, `mat_CM29INcREp6AWL0Ub`, `mat_7IjDUtOQa3TEmijN6`, `mat_5LKpXE7PvVA2ZaAgP`, `mat_E64JcLCA8Zq46lg8B`, `mat_HY8MBefMUAsLwtYX2`.
  - Batch review from challenger perspective: avg behavior score 36.8/100, `fix-no-pressure-loss` 11, `fix-no-value-created` 11, `risk-passive-fire` 11, no hard-constraint breach counted by the generic scorer. Boost analysis `/tmp/agentank-runs/shield-main/real15-v69-champion-top50/boost-analysis/boost-tactic-analysis.md` found 31 boost casts, 81 boost turns, 11 free-turn frames, 94 double-move frames, and 7 boost-turn shots with 3 hits and 3 walls.
  - Loss-frame review found a repeated sharper hazard: active bullets visible in replay were not reliably treated as active danger by the strategy. Examples: `mat_AeymrJcSnOGHrsUDT` turned in place for frames 49-52 while a left-moving visible bullet swept row 7; `mat_AkRsLSSIKLW5yJR9f` died to a boost mirror's first horizontal bullet after boost expired; `mat_HY8MBefMUAsLwtYX2` died to an overload offset bullet visible on the adjacent column. Local VM reproduction showed that strict future reply-line caution could reject the only immediately executable bullet-lane exit.
  - v70 patch stayed on one hazard axis. `active/shield-main.js` now normalizes active enemy bullets from both `enemy.bullet` and `game.visibleBullets`, uses them for ETA and long bullet-lane detection, disables enemy same-frame gunline assumptions while an active bullet exists, and lets urgent bullet-lane exits use immediate safety instead of full future reply-line strictness.
  - Regressions added: `shield-main v70 takes an urgent bullet-lane exit over future reply-line caution`; `shield-main v70 treats visible overload bullets as urgent lane danger`.
  - v70 gates passed: focused shield-main tests 30/30; `npm run test:lab`; `npm run check`; `git diff --check`; `.env npm run simulate` 9/9 at `/tmp/agentank-runs/simulations/2026-07-09T11-38-37-005Z`; `.env npm run challenge:dry` at `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-09T11-39-08-976Z.json`.
  - Published shield-main codeVersion 70, codeHash `aa77fdb800cae301719b2037ce09c91f4a6e1aeabdd2ed4bdfd1de40497c8dd3`. Publish snapshot `/tmp/agentank-runs/snapshots-shield-v70-publish/shield-main/tank.json` confirmed skillType boost, rankScore 1551, Master III, record 995-969-0, needsFix=false. The real run log ended at 1564, but the later live snapshot read 1551; use the snapshot for state.
  - Remaining top-50 issue after this patch is still pressure conversion against teleport and cloak-heavy Champion tanks, especially star losses with 0 shots. Do not widen this v70 hazard patch into attack logic without a separate replay cluster.
- Shield-main / tank 4839 `500` v69 fixed-shooter-line and boost back-shot publish plus real10:
  - User supplied three v68 samples: `mat_Ks36HLc1NyV7BUy6k`, `mat_8lroJuJahaf8pHkyN`, and `mat_Er0CsMDmuOVHeEHWb`. Review confirmed that boost can create same-frame action value, but the two losses had 0 own shots and exposed safety/pressure gaps: `mat_Ks36HLc1NyV7BUy6k` repeatedly showed a grass shooter at `[12,7]` via bullets, then 500 walked back onto that row and died; `mat_8lroJuJahaf8pHkyN` pressure-moved into a close one-turn reply lane; `mat_Er0CsMDmuOVHeEHWb` won by stars but had only one boost-turn shot and several missed boost pressure windows.
  - Local v69 patch stayed on one boost-frame-economy / shooter-line axis. `active/shield-main.js` now remembers fixed shooter lines exposed by enemy bullets, treats those lines as hard value-movement danger, uses strict value safety for fallback pressure steps, and adds active-boost `go + turn + fire` back-shot conversion when the boosted landing reaches the enemy's rear lane.
  - Regressions added: `shield-main v69 refuses a star on a remembered grass shooter line`; `shield-main v69 exits a remembered shooter row before value actions`; `shield-main v69 uses active boost go-turn-fire for a back shot`; `shield-main v69 does not pressure-step into a close one-turn reply lane`.
  - Gates passed before publish and real sample: `npm run check`; focused shield-main tests 28/28; `npm run test:lab`; `git diff --check`; `.env npm run simulate` 9/9 at `/tmp/agentank-runs/simulations/2026-07-09T11-19-27-910Z`; `.env npm run challenge:dry` at `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-09T11-19-28-845Z.json`.
  - Published shield-main codeVersion 69, codeHash `d2922fe2975b774e5e4ed18380ccfb4742f1bcf8aa905b0b252ce334077c70ca`. Publish snapshot `/tmp/agentank-runs/snapshots-shield-v69-publish/shield-main/tank.json` confirmed v69, skillType boost, rankScore 1387, Diamond II, record 978-950-0, needsFix=false.
  - User-requested real sample used `node lab/scripts/grind-adaptive-real.mjs --tank shield-main --limit 10 --max-wins-per-opponent 1 --drawdown-stop 80 --use-run-history --random-when-empty --output-dir /tmp/agentank-runs/shield-main/real10-v69-boost-backshot/raw --run-dir /tmp/agentank-runs/shield-main/real10-v69-boost-backshot/challenge-runs --execute`.
  - Real run log: `/tmp/agentank-runs/shield-main/real10-v69-boost-backshot/challenge-runs/2026-07-09T11-20-35-769Z.json`. Result: 10 settled matches, 6W/4L, run score 1403 -> 1491, no drawdown stop. Batch review: avg behavior score 41.8/100, fix signals `fix-no-pressure-loss` 4, `fix-no-value-created` 4, `fix-runtime-budget` 1, no hard-constraint breach counted.
  - Boost analysis: `/tmp/agentank-runs/shield-main/real10-v69-boost-backshot/boost-analysis/boost-tactic-analysis.md`. It found 19 boost casts, 43 turns during boost, 10 free-turn frames, 67 double-move frames, 1 boost-turn shot, and that shot hit. The new back-shot branch is safe but still rare in live play; do not keep widening it without more positive replay evidence.
  - Loss read: `mat_9gDZykCRAVtId7opJ` and `mat_4Z5IeH2Ge5lEQGeft` are stun/reversed bullet-return deaths after star or movement; `mat_KFCGGd7CEttKuADzf` is an overload close double-shot death; `mat_IEmN2y3T8QrJxbZUh` is freeze/runtime low pressure after only 1 wall shot. Treat these as a separate next-axis cluster, not more v69 boost-frame patching.
  - Post-run snapshot `/tmp/agentank-runs/snapshots-shield-v69-after-real10/shield-main/tank.json` confirmed codeVersion 69, codeHash `d2922fe2975b774e5e4ed18380ccfb4742f1bcf8aa905b0b252ce334077c70ca`, rankScore 1519, Master III, record 988-955-0, needsFix=false.
- Shield-main / tank 4839 `500` v68 publish:
  - User approved publishing the local v68 candidate after the known risk was disclosed.
  - Pre-publish context reread confirmed the workspace is scoped to tank 4839 `500`, active source `active/shield-main.js`, and the one-axis v68 hypothesis.
  - Publish gates rerun before release: `npm run check`; focused shield-main tests 24/24; `npm run test:lab`; `git diff --check`; `.env npm run simulate` 9/9 at `/tmp/agentank-runs/simulations/2026-07-09T10-57-17-043Z`; `.env npm run challenge:dry` at `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-09T10-57-49-156Z`, start score 1249.
  - Published shield-main codeVersion 68, codeHash `bac9b5cc63c01b89eee3789729b66e889445c795faa38421785a12a113a36aa6`.
  - Publish snapshot: `/tmp/agentank-runs/snapshots-shield-v68-publish/shield-main/tank.json`, skillType boost, rankScore 1271, Diamond III, record 964-940-0, needsFix=false.
  - No real challenge-run was executed after v68 publish. Next validation should be a bounded real observation comparing no-pressure losses, close reply counterfire, and whether adjacent safe-star preservation holds.
- Shield-main / tank 4839 `500` v65-v67 report review, v67 real10 follow-up, and local v68 over-defense/pressure candidate:
  - Read `active/ANALYSIS-v65-v67.md` as requested and removed it afterward. The report summarized 56 real matches: v65 13W/7L (+68), v66 8W/12L (-35), and v67 8W/8L (0). Its main warning was that v66/v67 broadened L0 too much and lowered the practical ceiling versus v65, while still leaving low attack output.
  - Pre-run gates on current local v67 shape passed: `npm run check`; focused shield-main tests 22/22; `npm run test:lab`; `git diff --check`; `npm run challenge:dry` at `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-09T10-42-24-873Z.json`. Server simulation was 8/9 at `/tmp/agentank-runs/simulations/2026-07-09T10-41-52-516Z`, with an `azure-hunter/arena` bullet-crash loss in the first review pass.
  - User-requested real sample used `node lab/scripts/grind-adaptive-real.mjs --tank shield-main --limit 10 --max-wins-per-opponent 1 --drawdown-stop 80 --use-run-history --random-when-empty --output-dir /tmp/agentank-runs/shield-main/real10-v67-followup/raw --run-dir /tmp/agentank-runs/shield-main/real10-v67-followup/challenge-runs --execute`.
  - Real run log: `/tmp/agentank-runs/shield-main/real10-v67-followup/challenge-runs/2026-07-09T10-42-53-115Z.json`. Result: 10 settled matches, 6W/4L, score 1127 -> 1167, peak 1187, no drawdown stop. Losses: `mat_FZ5zv1rXZWa1yCkYE`, `mat_IgQTTFou8bB0dunok`, `mat_BTCKGYh86XX2ToIK4`, and `mat_3iQyurMdoVlKqOqzA`.
  - Batch review from challenger perspective: avg behavior score 42.8/100, outcome categories 9 crash and 1 star_win. The 4 losses clustered at `fix-no-pressure-loss`, `fix-no-value-created`, and `risk-passive-fire`; the batch report did not count a hard-constraint cluster.
  - Event boost analysis: `/tmp/agentank-runs/shield-main/real10-v67-followup/boost-analysis/boost-tactic-analysis.md`. It found 20 boost casts, 38 turns during boost, 7 free-turn frames, 73 double-move frames, 5 boost-turn shots with 4 hits and 1 wall, and 11 stars collected within 8 frames after a boost turn. All 4 loss rows had 0 boost-turn shots and 0 hits, so the actionable cluster is still low pressure in losses rather than another broad defensive patch.
  - Loss-frame read: `mat_FZ5zv1rXZWa1yCkYE` died after collecting 3 stars but firing 0 shots; the final frame was a close same-row shot while 500 turned. `mat_IgQTTFou8bB0dunok` repeatedly exited a freeze tank's horizontal shot lane but never fired, then died to another left shot. `mat_BTCKGYh86XX2ToIK4` chased the star column through vertical bullet timing and died as the opponent collected. `mat_3iQyurMdoVlKqOqzA` collected a star, then died to a same-row freeze shot after another turn-only response.
  - Local v68 patch is one bounded balance axis. It narrows v67's broad generic L0 ranges: reply gunline default and value strictness use 11 cells instead of 13, current bullet escape triggers on 1 frame instead of 2, and close-gunline generic handling uses close 3 / gap 5 instead of close 5 / gap 7. It also preserves pressure by firing an already aimed close reply lane before generic gunline exit.
  - Regressions added: `shield-main v68 takes an adjacent safe star instead of fleeing a distant one-turn gunline`; `shield-main v68 fires an aimed close reply lane before generic gunline exit`.
  - Local v68 gates passed: focused shield-main tests 24/24; `npm run check`; `npm run test:lab`; `git diff --check`. Server simulation stayed 8/9 at `/tmp/agentank-runs/simulations/2026-07-09T10-50-04-412Z`; the remaining loss was `nova-scout/classic`, 0 shots, bullet crash, `hard-current-bullet-eta`. Shield dry-run passed at `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-09T10-50-38-081Z.json`, start score 1186.
  - Final live refresh: `/tmp/agentank-runs/snapshots-shield-v67-after-followup/shield-main/tank.json`, still codeVersion 67, codeHash `59ac805bbe87158a079266c84317e0a64cae61640e6d556bd787b048073f5be7`, rankScore 1186, Platinum I, record 959-939-0, needsFix=false. v68 is local only and should not be published without accepting or addressing the remaining 8/9 simulation risk.
- Shield-main / tank 4839 `500` user bullet-collision samples and local v65 candidate:
  - User supplied `mat_GVZEddpoyrYHnToGu`, `mat_D5MCjK5SrHkG9sWFF`, and `mat_44tsOEm7is76IW4gK`, reporting repeated direct bullet collisions and over-eager opening boost.
  - Replay package: `/tmp/agentank-runs/shield-v64-user-bullet-samples`, with summary, events, and frames views. Batch review was 0W/3L, avg behavior score 32/100, `fix-no-pressure-loss` 3, `fix-no-value-created` 3, `risk-passive-fire` 3.
  - Frame read:
    - `mat_GVZEddpoyrYHnToGu`: 500 cast boost on frame 3, collected the opening star on frame 11, then moved from `[2,12]` to `[2,11]` on frame 16 while an existing left-moving bullet swept `[3,11] -> [2,11]`, causing the crash.
    - `mat_D5MCjK5SrHkG9sWFF`: after a star pickup, 500 moved from `[8,8]` to `[9,8]` on frame 26 exactly as the opponent fired left from `[10,8]`; this is a same-frame close muzzle collision.
    - `mat_44tsOEm7is76IW4gK`: after collecting `[5,11]` on frame 34, 500 turned in place on frame 35 while the opponent fired down from `[5,9]`, sweeping `[5,10] -> [5,11]` and killing 500.
  - Local v65 patch stays on one hazard axis. `enemyBulletStepsTo()` now treats the current enemy bullet position as occupied danger. New same-frame close muzzle detection rejects cells that can be hit by a just-fired bullet within two cells. Survive now has `same-frame-shot-escape` before bullet/gunline/close-gunline layers, preferring an immediately executable `go` exit or an already-aimed trade before turn-only actions. Opening far-star boost now waits until frame 4, so 500 observes a few frames before spending boost on a distant star lane.
  - Regressions added: `shield-main v65 refuses to step onto the current enemy bullet tile`; `shield-main v65 drives out of a same-frame muzzle hit instead of turning`; `shield-main v65 does not step into a same-frame close muzzle lane`; `shield-main v65 waits a few opening frames before far-star boost`.
  - Gates passed after the patch: focused shield-main tests 22/22; `npm run check`; `npm run test:lab`; `git diff --check`; `.env npm run simulate` 9/9 at `/tmp/agentank-runs/simulations/2026-07-09T09-29-45-354Z`; `.env npm run challenge:dry` at `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-09T09-30-16-713Z.json`.
  - Live refresh after local work: `/tmp/agentank-runs/snapshots-shield-v64-live-after-v65-local/shield-main/tank.json`, still published codeVersion 64, rankScore 1081, Platinum II, record 913-897-0, needsFix=false.
  - Publish gates were rerun before release: `npm run check`; focused shield-main tests 22/22; `npm run test:lab`; `git diff --check`; `.env npm run simulate` 9/9 at `/tmp/agentank-runs/simulations/2026-07-09T09-38-44-814Z`; `.env npm run challenge:dry` at `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-09T09-39-22-550Z.json`, start score 1081.
  - Published shield-main codeVersion 65, codeHash `1f9892483418f8952644bb181be75f6d13c82b99dbd2e5d4fec34254e00ad97b`.
  - Publish snapshot: `/tmp/agentank-runs/snapshots-shield-v65-publish/shield-main/tank.json`, skillType boost, rankScore 1081, Platinum II, record 913-897-0, needsFix=false.
  - No real challenge-run has been executed after v65 publish. Next observation should specifically compare direct bullet stepping, same-frame close muzzle deaths, and whether delayed opening boost still preserves star tempo.
- Shield-main / tank 4839 `500` v64 recent-10 observation plus real10 evaluation:
  - Live refresh before the real run read codeVersion 64, codeHash `457e7678ed7b938ce0c219546a1c66f3b417ce4e41375d16c5ac750e9e9519bf`, rankScore 1044, Platinum II, record 902-888-0 at `/tmp/agentank-runs/shield-v64-observe-20260709/shield-main/tank.json`.
  - Recent visible 10 from the live match list were all v64 and went 4W/6L, net about -42. Batch review reported avg behavior score 39.2/100 with `fix-no-pressure-loss` 6, `fix-no-value-created` 6, and `risk-passive-fire` 6. Most of those very recent agent event views were still empty (`framesTotal=0`), so use this as result-level evidence only, not detailed frame evidence.
  - Gates before the real run passed: `npm run check`; `npm run test:lab`; `.env npm run simulate` 9/9 at `/tmp/agentank-runs/simulations/2026-07-09T09-15-44-802Z`; `.env npm run challenge:dry` at `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-09T09-16-20-299Z.json`.
  - Real command: `node lab/scripts/grind-adaptive-real.mjs --tank shield-main --limit 10 --max-wins-per-opponent 1 --drawdown-stop 80 --use-run-history --random-when-empty --output-dir /tmp/agentank-runs/shield-main/real10-v64-eval/raw --run-dir /tmp/agentank-runs/shield-main/real10-v64-eval/challenge-runs --execute`.
  - Real run log: `/tmp/agentank-runs/shield-main/real10-v64-eval/challenge-runs/2026-07-09T09-16-38-987Z.json`. Result: 10 settled matches, 7W/3L, score 1024 -> 1102, peak 1121, no drawdown stop. End snapshot: rankScore 1102, Platinum I, record 909-892-0, needsFix=false.
  - Batch review from challenger perspective: avg behavior score 44.6/100, categories 5 crash and 5 star, fix signals `fix-no-pressure-loss` 3 and `fix-no-value-created` 3, brave-baseline `risk-passive-fire` 3, no hard-constraint breach reported.
  - Full event boost analysis: `/tmp/agentank-runs/shield-main/real10-v64-eval/boost-analysis/boost-tactic-analysis.md`. It found 17 boost casts, 31 turns during boost, 10 free-turn frames, 61 double-move frames, 2 shots after boost-turn setup (1 hit / 1 wall), and 8 stars collected within 8 frames after a boost turn. This is a clear improvement over v61-v63: boost now creates star value, but still converts too little into attack.
  - Loss read: `mat_9zcvKLB8erjD00tCC` collected a star at frame 29 but did not exit the reply lane; enemy fired down and killed 500 at frame 30. `mat_Lp9Z6fZqS8fDXPrik` used boost to cross the map and collect a star, then fired two wall shots into a shield opponent and died to the reply shot. `mat_8Ee3ua6gRBRFmefOc` lost a fast shield matchup star race, fired three times with two wall shots, and ended in a mutual crash while already losing the star point.
  - Score for this version after the run: ladder result B (7W/3L, +78), boost-star conversion B+ (8 boost-linked star pickups), boost-attack conversion C- (only 2 boost-turn shots and 1 hit), wall-shot discipline D (4 wall shots in the 3 losses), post-star lane safety C- (fresh star pickup still died to immediate reply in one loss), overall current strategy grade C+/B- at the 1000-1100 band. Do not publish or patch from a single loss; the next bounded axis should be shield/close reply lane wall-shot and post-star exit quality if this repeats in the next sample.
- Shield-main / tank 4839 `500` recent-10 review and local v64 hazard guard candidate:
  - User reported a losing streak, weak dodging, and poor obstacle/gunline understanding while asking to borrow suitable survival judgment from Dark Edge without making the tank passive.
  - Live refresh before patch read codeVersion 63, codeHash `4b93a72982deb7e4aa3cd986a44024a7c3d516dc591a9e62b201eee05e76ccbb`, rankScore 1236, Diamond III, record 895-871-0 at `/tmp/agentank-runs/shield-v63-latest10-20260709/shield-main/tank.json`.
  - Recent visible 10 review: 3W/7L overall. Only the latest 5 were v63 and they went 1W/4L. Batch review clustered around `fix-no-pressure-loss` and `fix-no-value-created`; frame/event review showed repeated same-line/long-lane danger, wall shots, and fallback movement continuing into unsafe lines after a value move was rejected.
  - Boost tactic analysis for those 10: 17 boost casts, 29 turns during boost, 8 free-turn frames, 62 double-move frames, and 3 shots after boost-turn setup, but 0 hits and 2 wall shots. v63 improved boost mechanics but still failed to convert enough pressure and still bled to gunline hazards.
  - Local v64 patch is one hazard axis only. `active/shield-main.js` now has small Dark Edge style helpers for enemy debuff state, reply-capable gunline direction, long bullet lanes, lane exits, short hazard-exit commitment, and `poorGunlineAt`. Strict `cellSafe()` now rejects long bullet lanes and reply-capable gunlines; value movement uses the same reply-gunline guard before a shot exists; `survive` adds `gunline-escape` before close-gunline logic. L8 fallback now also respects `cellSafe()` so a rejected unsafe star path cannot be re-entered through `center`.
  - Aggression is preserved: boosted clear-lane snap fire still outranks generic gunline escape; active boost gunline landing uses non-strict safety to keep `go + turn` attack value; close/no-exit aimed duels still fire instead of turning away.
  - Regressions added: `shield-main v64 exits a reply-capable same-row gunline before value movement`; `shield-main v64 refuses to step into a long bullet lane while chasing a star`; `shield-main v64 does not boost-step into a reply-capable edge gunline for star value`; `shield-main v64 treats a recently seen hidden enemy line as reply-capable`; `shield-main v64 commits to a selected bullet-lane exit instead of oscillating`; `shield-main v64 still fires an aimed same-line trade when no line exit exists`.
  - Final gates passed before publish: focused shield-main tests 18/18; `npm run check`; `npm run test:lab`; `git diff --check`; `.env npm run simulate` 9/9 at `/tmp/agentank-runs/simulations/2026-07-09T08-55-31-655Z`; `.env npm run challenge:dry` at `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-09T08-56-23-801Z.json`.
  - Published shield-main codeVersion 64, codeHash `457e7678ed7b938ce0c219546a1c66f3b417ce4e41375d16c5ac750e9e9519bf`.
  - Publish snapshot: `/tmp/agentank-runs/snapshots-shield-v64-hazard-guard-publish/shield-main/tank.json`, skillType boost, rankScore 1127, Platinum I, record 898-880-0, needsFix=false.
  - No real challenge-run has been executed after v64 publish. Next observation should compare bullet-death frequency, hidden/recent gunline exits, and whether boost still converts into `turn + fire`, `go + turn`, and `turn + go` value.
- Shield-main / tank 4839 `500` v63 active-boost free-turn publish:
  - Scope is one boost-mechanics axis only: convert active boost into explicit `turn + go` on valuable star routes, while keeping the existing `turn + fire` snap-shot and `go + turn` gunline landing behaviors.
  - `active/shield-main.js` now has shared boost compound-action helpers: `boostTurnGo()` for free-turn star movement and `boostGoTurn()` for landing into a gunline. `tryActiveBoostStarStep()` uses `boostTurnGo()` only when the star path is farther than two cells, so protected near-star control still avoids overshooting.
  - Regression added: `shield-main v63 uses active boost free turn then go toward a valuable star route`.
  - Gates passed before publish: `npm run check`; focused shield-main tests 12/12; `git diff --check`; `npm run test:lab`; `.env npm run simulate` first pass 8/9 at `/tmp/agentank-runs/simulations/2026-07-09T08-09-14-584Z` with a `crimson-bastion/random` bullet-crash loss reviewed as `hard-current-bullet-eta`; rerun passed 9/9 at `/tmp/agentank-runs/simulations/2026-07-09T08-10-40-294Z`; shield-main dry-run `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-09T08-11-23-436Z.json` with start score 1298.
  - Published shield-main codeVersion 63, codeHash `4b93a72982deb7e4aa3cd986a44024a7c3d516dc591a9e62b201eee05e76ccbb`.
  - Publish snapshot: `/tmp/agentank-runs/snapshots-shield-v63-publish/shield-main/tank.json`, skillType boost, codeVersion 63, rankScore 1298, Diamond III, record 894-867-0.
  - No real challenge-run has been executed after v63 publish. Next validation should be a small stop-loss batch and should measure active-boost `turn + go`, `go + turn`, and `turn + fire` conversion rates separately from current-bullet deaths.
- Shield-main / tank 4839 `500` v62 publish:
  - Published shield-main codeVersion 62, codeHash `15f2cf1c281b1493ef2969121864be121fbb7946b0e123352d5b241403121d3f`.
  - Gates passed before publish: `npm run check`; focused shield-main tests 11/11; `git diff --check`; `npm run test:lab`; `.env npm run simulate` 9/9 at `/tmp/agentank-runs/simulations/2026-07-09T07-47-44-570Z`; shield-main dry-run `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-09T07-48-20-265Z.json` with start score 1339.
  - Publish snapshot: `/tmp/agentank-runs/snapshots-shield-v62-publish/shield-main/tank.json`, skillType boost, codeVersion 62, rankScore 1339, Diamond II, record 891-862-0.
  - No real challenge-run has been executed after v62 publish. Next run should be small and bounded, and should specifically compare current-bullet deaths and `hard-current-bullet-eta` frequency against the v61 real observation.
- Shield-main / tank 4839 `500` v61 real observation and local v62 current-bullet exit candidate:
  - User requested a 10-match look after observing v61 was more aggressive and roughly even by eye. Gates before real run: `npm run check`; focused shield-main tests 10/10; `npm run test:lab`; `.env npm run simulate` was 8/9 at `/tmp/agentank-runs/simulations/2026-07-09T07-14-46-862Z` with one `azure-hunter/random` crash loss; shield-main dry-run passed at `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-09T07-15-34-485Z.json`.
  - Real command used the adaptive runner with `--limit 10`, `--max-wins-per-opponent 1`, `--drawdown-stop 80`, `--use-run-history`, `--random-when-empty`, and `--execute`: `/tmp/agentank-runs/shield-main/real10-v61-observe/challenge-runs/2026-07-09T07-16-04-328Z.json`.
  - Result: stopped after 9/10 by drawdown, 2W/7L, run score 1435 -> 1338, peak 1435, end 1338. Latest snapshot after the run: `/tmp/agentank-runs/snapshots-shield-v61-after-real10-v62-local/shield-main/tank.json`, codeVersion 61, codeHash `451dd8d4aad04ea8a585930430a11254b4c44755c9dd8d3d8aee4b4cc7f1ecad`, rankScore 1319, Diamond II, record 888-861-0.
  - Replay package: summaries in `/tmp/agentank-runs/shield-main/real10-v61-observe/raw`, events in `/tmp/agentank-runs/shield-main/real10-v61-observe/events`, raw replays in `/tmp/agentank-runs/shield-main/real10-v61-observe/raw-replay`, death frames in `/tmp/agentank-runs/shield-main/real10-v61-observe/frames`.
  - Batch review from challenger perspective: 9 matches, 2W/7L, avg behavior score 15.3/100, all 9 categorized as bullet_crash in replay evidence; fix signals clustered at `fix-bullet-death` 7, `fix-post-shield-reset` 7, `fix-shield-no-conversion` 7, `fix-lost-initiative` 6, and `fix-no-pressure-loss` 6. Hard constraint breaches: `hard-current-bullet-eta` 7.
  - Boost tactic analysis: `/tmp/agentank-runs/shield-main/real10-v61-observe/boost-analysis/boost-tactic-analysis.md`. It found 10 boost casts, 12 turns during boost, 0 free-turn frames, 35 double-move frames, 0 boost-turn shots, and 3 stars collected within 8 frames after a boost turn. v61 is more aggressive than the passive line, but it still does not convert boost into the intended free-turn attack value.
  - Local v62 patch: bounded to one hazard axis. `active/shield-main.js` now computes enemy bullet step distance and, when current bullet danger exists, prefers an immediately executable `go` exit before turn-only escape. `safeExitFromLane()` also prefers already-aligned safe exits inside the same threat family. This does not change boost timing, star routing, or assault priority.
  - Regression added: `shield-main v62 immediately goes out of a current bullet lane when already aligned`.
  - Local candidate gates after the patch: focused shield-main tests 11/11; `npm run check`; `npm run test:lab`; `.env npm run simulate` 9/9 at `/tmp/agentank-runs/simulations/2026-07-09T07-21-59-352Z`; shield-main dry-run `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-09T07-22-28-402Z.json` with start score 1319. Candidate is local only; no publish has been executed.
- Shield-main / tank 4839 `500` v61 boost-only reset publish:
  - Published shield-main codeVersion 61, codeHash `451dd8d4aad04ea8a585930430a11254b4c44755c9dd8d3d8aee4b4cc7f1ecad`.
  - Gates passed before publish: `npm run check`; focused shield-main tests 10/10; `node --check lab/scripts/analyze-boost-tactics.mjs`; `npm run test:lab`; `git diff --check`; `.env npm run simulate` 9/9 at `/tmp/agentank-runs/simulations/2026-07-09T06-55-13-194Z`; shield-main dry-run `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-09T06-55-42-486Z.json` with start score 1451.
  - Publish snapshot: `/tmp/agentank-runs/snapshots-shield-v61-boost-reset-publish/shield-main/tank.json`, skillType boost, codeVersion 61, rankScore 1451, Diamond I, record 883-849-0.
  - No real challenge-run has been executed after v61 publish. Next run should be small and bounded, and should explicitly measure whether active boost produces free-turn value and whether close same-row/same-column crash losses drop.
- Shield-main / tank 4839 `500` v60 diagnosis and local v61 boost-only reset:
  - User supplied v60 losses `mat_5oTFMFEXjVP9JdP3l` and `mat_5ToUOWNv2pKLDodyZ`, reporting that boost was still being cast without purpose and movement looked random.
  - User sample package: `/tmp/agentank-runs/shield-v60-user-samples`. Boost tactic analysis over the 2 losses: 0W/2L, 8 boost casts, 21 turns during boost, 0 free-turn frames, 0 boost-turn shots, and 0 stars collected within 8 frames after a boost turn. Summary: v60 did not implement the intended 开心果-style active boost free-turn value.
  - Ran a bounded hard diagnostic with target 10 and `drawdownStop=80`, not one-loss stop: `/tmp/agentank-runs/shield-main/hard10-v60-diagnosis/challenge-runs/2026-07-09T06-42-13-953Z.json`. It stopped after 6 games by drawdown: 1W/5L, score 1552 -> 1471, all settled by crash. Replay events/frames are under `/tmp/agentank-runs/shield-main/hard10-v60-diagnosis/events` and `frames`.
  - v60 hard10 boost analysis: 12 boost casts, 27 turns during boost, 0 free-turn frames, 30 double-move frames, only 1 boost-turn shot setup and that shot hit a wall. Five losses included repeated short crash shapes: near same-row/same-column gunline deaths, poison/cloak/overload pressure, and 0-shot or low-shot games.
  - Decision: stop patching the old 3000-line shield/boost mixed stack. Replaced `active/shield-main.js` with a 501-line boost-only behavior tree candidate. The new v61 local strategy has four selectors: survive, assault, star, pressure. Boost is only legal for aligned valuable star routes or explicit gunline creation; active boost can same-frame turn+fire or go+free-turn into a gunline.
  - Focused tests were reset to the new contract: 10/10 passed. Training space and CURRENT docs were updated to the v61 boost-reset axes and protected behaviors.
  - Gates passed for local v61 candidate: `npm run check`; `npm run test:lab`; `git diff --check`; `.env npm run simulate` 9/9 at `/tmp/agentank-runs/simulations/2026-07-09T06-51-05-312Z`; shield-main dry-run `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-09T06-51-36-902Z.json`.
  - Live snapshot after diagnosis, before any v61 publish: `/tmp/agentank-runs/snapshots-shield-v60-after-hard10-v61-local/shield-main/tank.json`, still codeVersion 60, codeHash `aec597e718d5f179d461a8e4bde93e6eab77a45597e00f0555d60b410345277a`, skillType boost, rankScore 1451, Diamond I, record 883-849-0. v61 is local only and has not been published.
- Shield-main / tank 4839 `500` v60 publish and real stop-loss:
  - Published active boost snap-fire candidate as shield-main codeVersion 60, codeHash `aec597e718d5f179d461a8e4bde93e6eab77a45597e00f0555d60b410345277a`.
  - Publish gates passed in this run: `npm run check`; focused shield-main tests 138/138; `npm run test:lab`; `git diff --check`; `.env npm run simulate` 9/9 at `/tmp/agentank-runs/simulations/2026-07-09T06-27-59-366Z`; shield-main dry-run `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-09T06-27-59-945Z.json` with start score 1615, `stopOnLoss=true`, and `drawdownStop=25`.
  - Publish snapshot: `/tmp/agentank-runs/snapshots-shield-v60-boost-snap-publish/shield-main/tank.json`, skillType boost, codeVersion 60, rankScore 1615, Master II.
  - Real validation command targeted 10 matches with climb-policy and stop-on-loss, but stopped after the first settled loss: `/tmp/agentank-runs/shield-main/real10-v60-boost-snap/challenge-runs/2026-07-09T06-29-32-186Z.json`.
  - Result: 0W/1L, `mat_8pOQhpkJmqVDuqDo8` vs `silver_wolf` #4205, crashed, score 1615 -> 1594, Master III. Raw/agent summary: `/tmp/agentank-runs/shield-main/real10-v60-boost-snap/raw/mat_8pOQhpkJmqVDuqDo8.json`; events: `/tmp/agentank-runs/shield-main/real10-v60-boost-snap/events/mat_8pOQhpkJmqVDuqDo8.events.json`; frames: `/tmp/agentank-runs/shield-main/real10-v60-boost-snap/frames/`.
  - Tactical read: 500 had 2 boost casts, 5 turns during boost, 0 free-turn frames, 0 boost-turn shots, 2 stars, and 1 wall shot. The v60 snap-fire mechanism did not trigger in this match. Death sequence: opponent collected the `[6,6]` star at frame 78 while 500 was at `[6,4]`; 500 stayed in the same column and turned right on frames 79-80; opponent fired upward on frame 80 and hit 500. This is a candidate close same-column post-star gunline issue, but by method it needs more real samples before a new patch.
- Shield-main / tank 4839 `500` local boost snap-fire candidate from targeted 开心果 research:
  - Targeted source: public tank card `https://agentank.ai/share/tanks/tnk_3JfEhhcXu7S4kgIzA`, tank `开心果`, Champion, Boost, codeVersion 410, rankScore 2704 at fetch time.
  - Crawled 40 public recent matches to `/tmp/agentank-runs/boost-target-tnk-3JfEhhcXu7S4kgIzA`, including event views under `events/` and frame views under `frames/`. Aggregate event report: `/tmp/agentank-runs/boost-target-tnk-3JfEhhcXu7S4kgIzA/analysis/boost-tactic-analysis.md`. Coordinate report: `/tmp/agentank-runs/boost-target-tnk-3JfEhhcXu7S4kgIzA/coordinate-analysis/coordinate-analysis.md`.
  - Research result: 开心果 sample was 26W/14L, with 90 boost casts, 303 turns during boost, 211 free-turn frames, 300 double-move frames, and 31 boost-turn shot setups in event analysis. Coordinate analysis confirmed 116 active-boost frames with same-frame go+turn, 5 active-boost turn+fire frames, 28 boost-linked shots, and 27/59 star pickups from an adjacent previous frame.
  - Key mechanism verified in `mat_E7Xcl26SzioHzVYpQ`: frame 39 has same-frame `turn` plus bullet creation from 开心果, and frame 40 the shot hits the opponent. This proves the boost window can convert a free turn into immediate fire; our previous modules mostly turned first and waited a later frame.
  - Loss read: 开心果 still loses on star tempo and wall shots, especially versus teleport. The transferable lesson is not "shoot more always"; it is bounded snap-fire only when active boost, clear lane, no live bullet/bomb/overload/hidden hard danger, and no protected close-star pickup is being stolen.
  - Local patch: added `boostTurnFireIfSafe()` and `tryBoostSnapCounterShot()` in `active/shield-main.js`. Existing boost confirmed-shot, boost free-turn ambush, and boost initiative pressure can now issue same-frame turn+fire on safe clear lanes. L0 `tryEmergencyDefense()` now allows boost snap-fire only in the aimed-but-not-yet-hit window; live bullets and overload/hidden danger still keep defensive priority.
  - Regressions added/updated: `shield-main uses boost free-turn ambush while boosted on a clear lane` now expects turn+fire; `shield-main uses boosted snap fire before panic dodge when only aimed`; `shield-main does not snap fire over a live bullet crossing the current tile`.
  - Gates passed: focused shield-main tests 138/138; `npm run check`; `npm run test:lab`; `git diff --check`; `.env` server simulation 9/9 at `/tmp/agentank-runs/simulations/2026-07-09T06-21-03-694Z`; shield-main dry-run `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-09T06-21-44-876Z.json` with start score 1615 and stop-on-loss policy. Candidate is local only; no publish and no real challenge-run has been executed yet.
- Shield-main / tank 4839 `500` v59 direct safe star-line guard:
  - User supplied v58 live losses `mat_9SPWRPfm8as1Ak7vH`, `mat_AozORVBpPIn4oeQGJ`, and `mat_L7YI2L2OJRfKgOAFx`, reporting that 500 kept giving stars away, ran when seeing opponents, and failed to turn boost into attacks.
  - Replay package: `/tmp/agentank-runs/shield-user-v58-complaints`. Batch review from challenger perspective was 0W/3L, avg behavior score 32/100, with `fix-no-pressure-loss` 3, `fix-no-value-created` 3, and `risk-passive-fire` 3.
  - Boost tactic analysis confirmed the core regression: 9 boost casts, 18 turns during boost, 0 free-turn frames, 0 boost-turn shots, and only 1 star within 8 frames after a boost turn. This means v58 was still converting boost mostly into side movement, not value.
  - Exact frame review found safe direct star lines being swallowed by lower-value pressure: in `mat_9SPWRPfm8as1Ak7vH`, a direct `[5,7]` star line at frame 59 was interrupted by `boost-free-turn-ambush` side/back routing; in `mat_AozORVBpPIn4oeQGJ`, a clear `[2,5]` star line around frame 86 was routed through `boost-pressure` instead of a star-line commit.
  - Bounded patch: added `directSafeStarRouteShouldLead()` and `tryDirectSafeStarRouteLead()` in `active/shield-main.js`. Active/recent boost now stays on a safe direct star line before side/back ambush; when boost is unavailable, 500 continues a safe direct star line if it is already facing the route. Boost-ready valuable star routes still keep the existing boost cast behavior, and current clear enemy gunlines still keep the existing fire/turn behavior before this guard.
  - Regressions added: `shield-main keeps active boost on a direct safe star line before side-back ambush`; `shield-main follows a direct safe star line before boost side pressure`.
  - Gates before publish: focused shield-main tests 136/136; `npm run check`; `npm run test:lab`; `git diff --check`; shield-main dry-run `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-09T05-59-03-938Z.json`; server simulation first pass 8/9 at `/tmp/agentank-runs/simulations/2026-07-09T05-59-03-212Z`, with the single `azure-hunter/classic` bullet-lane crash classified as a separate hazard-first issue after 500 led 2-0; server simulation rerun passed 9/9 at `/tmp/agentank-runs/simulations/2026-07-09T06-00-52-694Z`.
  - Publish: shield-main codeVersion 59, codeHash `769ad11b65639e6797042ecfe322ec1a42b214357756d2b16d003b298acee969`. Snapshot `/tmp/agentank-runs/snapshots-shield-v59-direct-star-publish/shield-main/tank.json` confirmed skillType boost, rankScore 1636, Master II, record 878-835-0. No real challenge-run was executed after publish.
  - Decision: v59 fixes the immediate priority conflict but does not prove ladder stability. Next live observation should review post-v59 replays for two separate clusters: whether direct safe star lines are now collected, and whether the bottom-edge bullet-lane escape seen in `azure-hunter/classic` repeats often enough to become the next hazard-first axis.
- Shield-main / tank 4839 `500` v57-v58 active-boost free-turn attack validation:
  - User reported boost still did not use active-turn mechanics well and asked to raise priority for using boost to reach the opponent's side/back and shoot, then run 10 real battles.
  - Pre-v57 review of post-v56 event replays found the problem was real: 5 visible post-v56 matches had 13 boost casts and 31 turns during boost, but 0 boost-turn shots. The tank was still converting boost mostly into movement/stars, not pressure.
  - v57 bounded patch: extended L2 `boost-free-turn-ambush` so active boost can cut toward a high-value side/back gun lane when no safe close star or hard danger exists. Regressions added: `shield-main uses boost free-turn ambush to cut toward a side-back gun lane`. A small runtime bug in `postStarExitAvailable()` was also fixed from `n[0]` to the real `x/y` coordinates.
  - v57 gates passed: focused shield-main tests 133/133, `npm run check`, `npm run test:lab`, `git diff --check`, server simulation 9/9 at `/tmp/agentank-runs/simulations/2026-07-09T03-51-51-316Z`, and dry-run `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-09T03-52-30-420Z.json`. Publish: codeVersion 57, codeHash `aeb827fafc90926cef88811fc312bdef3fbaf43d1274d8bfe9eb7a25842da4c3`.
  - v57 real validation used `drawdownStop=25`, `stopOnLoss=false`, target 10. It stopped after 2 losses: `/tmp/agentank-runs/shield-main/real10-v57-boost-ambush/challenge-runs/2026-07-09T03-52-53-252Z.json`, score 1813 -> 1777, matches `mat_5DLwXAR5ekxDOvwgJ` and `mat_5X36KWzRJXQ2Rd8i6`.
  - v57 event analysis: `/tmp/agentank-runs/shield-main/real10-v57-boost-ambush/events/boost-tactic-analysis.md`, 8 boost casts, 17 turns during boost, 0 boost-turn shots, 0 own shots in the two-match summary. Exact frame review of `mat_5DLwXAR5ekxDOvwgJ` showed the new side/back entry did trigger at frame 36 (`免费转向切后排`) but used a normal boosted move to enter the row, then only turned next frame; the enemy moved off the lane before 500 could fire.
  - v58 bounded patch: when an active boost landing creates a clean gunline, `boost-free-turn-ambush` can issue same-frame `go` + `turn` to face the next-frame shot. Regression added: `shield-main uses boost free-turn to face after landing on a gun lane`.
  - v58 gates passed: focused shield-main tests 134/134, `npm run check`, `npm run test:lab`, `git diff --check`, server simulation 9/9 at `/tmp/agentank-runs/simulations/2026-07-09T03-58-58-273Z`, and dry-run `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-09T03-59-36-644Z.json`. Publish: codeVersion 58, codeHash `51cca712020d54e7c02dc6aa68265eaa7d80c491ebb261fae5c0853b5163b10a`.
  - v58 real validation also stopped after 2 losses by drawdown: `/tmp/agentank-runs/shield-main/real10-v58-boost-free-turn/challenge-runs/2026-07-09T03-59-52-845Z.json`, score 1757 -> 1717, matches `mat_Kid0KfN8AYM5IYBM6` and `mat_Ap9Ad2ehUQL4iCh6A`. Snapshot after run: `/tmp/agentank-runs/snapshots-shield-v58-after-real/shield-main/tank.json`, skillType boost, codeVersion 58, rankScore 1717, Master I, record 878-831-0.
  - v58 event analysis: `/tmp/agentank-runs/shield-main/real10-v58-boost-free-turn/events/boost-tactic-analysis.md`, 4 boost casts, 7 turns during boost, 0 free-turn frames, 0 boost-turn shots. It did improve from total zero shooting to 1 shot in `mat_Kid0KfN8AYM5IYBM6`, but still failed the intended boost-turn-shot objective and went 0W/2L.
  - Decision: stop real challenges on this candidate line. Next round must be offline replay-driven: prove actual free-turn frames and boost-turn shots in event analysis before another real run. Do not keep patching broad star/grass/pressure ideas into this same axis.
- Shield-main / tank 4839 `500` v56 publish after review-gated boost behavior-tree candidate:
  - User requested review and publish if clean. Pre-publish code review kept scope to `active/shield-main.js` and shield-main state; existing Dark Edge dirty files were not published or mixed into this release.
  - Candidate behavior: behavior-tree selector scheduling over the existing strategy modules; bounded L2 active-boost `boost-free-turn-ambush` after confirmed shot and before initiative pressure; safe close stars, hard danger, landing safety, and one-turn enemy reply gates remain protected.
  - Review caught two unsafe simulation signals before release: first standard simulation was 8/9 at `/tmp/agentank-runs/simulations/2026-07-09T03-30-25-135Z` with a `crimson-bastion/classic` hard bullet crash; rerun was 8/9 at `/tmp/agentank-runs/simulations/2026-07-09T03-33-05-109Z` with a `nova-scout/random` runtime loss where 500 got stuck around `[3,7]` and kept turning.
  - Bounded safety patch before publish: L8 `tryUnstick()` now uses `unstickEscapeDir()` / `unstickEscapeCellSafe()` so repeated stuck frames choose a real open escape cell before falling back to turn-only spinning. Regressions added: `shield-main unstick walks out of a two-sided pocket instead of spinning`; `shield-main keeps unstick escape movement before turn-only fallback`.
  - Gates passed after the guard: `node --check active/shield-main.js`; focused shield-main tests 132/132; `git diff --check`; JSON state parse; `npm run check`; `npm run test:lab`; server simulation 9/9 at `/tmp/agentank-runs/simulations/2026-07-09T03-39-08-771Z`; shield-main dry-run `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-09T03-39-49-808Z.json` with start score 1831 and stop-on-loss policy.
  - Publish: shield-main codeVersion 56, codeHash `9cbaa27b66a34167ad1b9afd23bf022f5e8783ecb7c078b17c272e5c5c4c29a2`.
  - Live snapshot after publish: `/tmp/agentank-runs/snapshots-shield-v56-publish/shield-main/tank.json`, skillType boost, rankScore 1831, Champion I, record 876-823-0, codeVersion 56 confirmed. No real challenge-run was executed.
- Shield-main / tank 4839 `500` rollback to boost and local behavior-tree free-turn ambush candidate:
  - User reported that 500 had manually copied Dark Edge overload code while keeping boost skill, climbed to Champion, but should return to a real boost strategy. Live refresh confirmed this mixed state: skillType `boost`, codeVersion 54, codeHash `f202de6b9c9b60c263404df404979d70afc3d2af2435bfe1a1e7074e78b5500f` (Dark Edge v91 hash), rankScore 1992.
  - Rollback gates passed before publishing the previous boost baseline: `npm run check`, `npm run test:lab`, `git diff --check`, server simulation 9/9 at `/tmp/agentank-runs/simulations/2026-07-09T02-29-18-908Z`, and shield dry-run `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-09T02-29-19-533Z.json`.
  - Rollback publish: shield-main codeVersion 55, codeHash `ac09e1380bb8bfd8c8784713bfee3acc433411f0d42093e0144a780fed002cc8`, restoring the v53 boost baseline hash. Immediate rollback snapshot `/tmp/agentank-runs/snapshots-shield-v55-rollback-boost/shield-main/tank.json` read rankScore 1992; final refresh after candidate gates `/tmp/agentank-runs/snapshots-shield-v55-after-candidate-gates/shield-main/tank.json` reads rankScore 1879, Champion I, record 874-818-0, still codeVersion 55.
  - Latest boost leaderboard probe found `开心果` as boost rank 1, tankId 3081, rankScore 2761, codeVersion 408. 500 was boost-filtered rank 9 at score 1992 after rollback.
  - Crawled `开心果` latest 40 boost matches to `/tmp/agentank-runs/boost-kxg-20260709/2026-07-09T02-30-12-091Z`. Summary: 26W/14L, 35/40 opening boost within 3 frames, 38/40 boosted before first own star, 142 boost casts, 123 stars, 162 shots.
  - Event tactic analysis `/tmp/agentank-runs/boost-kxg-20260709/2026-07-09T02-30-12-091Z/boost-tactic-analysis.md`: 499 turns during boost, 310 free-turn frames while boosted, 45 boost-turn shot setups, 5 hits and 39 wall shots, plus 46 stars collected within 8 frames after a boost turn. The useful tactic is not random turning; it is active-boost turn/fire only when the lane is clear and the enemy cannot answer within one turn.
  - Local candidate: added `lab/scripts/analyze-boost-tactics.mjs` for reusable event replay analysis; added a behavior-tree selector scheduler over the existing L0-L8 modules; added bounded L2 `boost-free-turn-ambush` between confirmed shot and initiative pressure. It fires or turns only during active boost, with hard danger, safe close-star, landing safety, and one-turn enemy reply gates.
  - Regressions added: `shield-main uses boost free-turn ambush while boosted on a clear lane`; `shield-main does not spend boost free-turn ambush over a safe adjacent star`. Candidate gates passed: focused shield-main tests 130/130, `npm run check`, `npm run test:lab`, `git diff --check`, JSON state parse, server simulation 9/9 at `/tmp/agentank-runs/simulations/2026-07-09T02-41-30-501Z`, and dry-run `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-09T02-41-31-194Z` with live start score 1879. Candidate is local only; no publish or real challenge-run was executed after this local candidate.
- Dark Edge / v91 latest-40 real review and local close-aimed edge-trap candidate:
  - Live snapshot from the review window confirmed Dark Edge codeVersion 91, codeHash `f202de6b9c9b60c263404df404979d70afc3d2af2435bfe1a1e7074e78b5500f`, rankScore 1801, public rank 137, record 8218-7307-0. Compared with the v91 publish snapshot at 2360, the overnight drawdown is about 559 points; the latest 40 visible matches themselves went 33W/7L but still net -47 because wins were small and low-score losses were expensive.
  - Latest-40 package: `/tmp/agentank-runs/dark-edge-latest40-20260709`, with raw replays in `raw-replay/`, event views in `events/`, ids in `ids.txt`, and aggregate summary in `dark-edge-40-summary.json`.
  - Result shape: 33W/7L, win rate 82.5%, but score economy was poor. The largest cluster was vs `无敌虾` (30 games, 26W/4L, net -127) because each win was tiny while each loss cost around 38-39 points. Preserve behavior is still overload/clear-kill pressure: 33 wins were mostly crash wins with pressure conversion.
  - Follow-up overnight pull confirmed the real drawdown window was 2026-07-09 01:00-06:59 Beijing time: 110 matches, 62W/48L by rank delta, net -873. The heaviest 04:00-06:59 window was 92 matches, 62W/30L, net -670. Replay package: `/tmp/agentank-runs/dark-edge-overnight-20260709`, with role-aware summary at `heavy-0406-analysis-summary-role-aware.json`.
  - 04:00-06:59 score drain by opponent: `无敌虾#2239` 66 games, 52W/14L, net -535; `KAI#1905` 18 games, 6W/12L, net -130; `金色飞贼-Mira#5076` 3 games, 0W/3L, net -61. This explains the apparent contradiction: win count was high, but low-score opponent losses destroyed ladder score.
  - 04:00-06:59 behavior clusters: preserve `preserve-clear-kill-pressure` 56 and `preserve-skill-tempo` 20; fix `fix-star-tempo-loss` 63, `fix-bullet-death` 24, `fix-no-pressure-loss` 23, `fix-no-value-created` 20; hard `hard-current-bullet-eta` 24, `hard-close-aimed-duel` 2, `hard-breakable-cover-shot` 1.
  - Interpretation under bounded evolution: do not mix KAI/Mira star-tempo and enemy-overload-offset fixes into the current patch. The active local axis remains the largest hard-loss subset inside the biggest score sink: close/edge aimed gunline deaths against `无敌虾`. KAI enemy-overload offset no-exit and Mira teleport-star tempo are logged as next-round axes after this candidate is validated or rejected.
  - Loss clusters: `mat_DhCybimkgU34FmCMJ`, `mat_G40IaGjI0E2LtDcUa`, and `mat_1v56Lge9oQi5w573p` repeated close/edge hard-current-bullet failures after star pressure; `mat_3O7ZM4aHixOBmSyk0` was a close aimed mutual crash loss; `mat_7Sgwr1ubGztFKwK6K` was a late star-loop/no-skill loss; `mat_1jgkLRPiahC121Vki` was runtime/no-pressure. The bounded fix axis is the repeated close aimed no-exit failure, not broad star-tempo or grass rewrites.
  - Patch: `tryEmergencyDefense()` now checks `tryNoExitCloseAimedCounter()` before generic dodge. If there is a one-frame safe exit, Dark Edge still exits. If no such exit exists, the enemy is visible/unshielded, fire is ready, distance is close, and the line is clear, Dark Edge fires or turns toward the enemy instead of panic-turning into the edge.
  - Regression added: `dark-edge turns back to counter when a close aimed edge trap has no one-frame exit`.
  - Publish gates passed: `npm run check`; focused Dark Edge tests 54/54; `npm run test:lab`; `git diff --check`; Dark Edge training-space coverage check; Dark Edge dry-run `/tmp/agentank-runs/dark-edge/challenge-runs/2026-07-09T02-51-51-615Z.json` with start score 1863; standard `npm run simulate` 9/9 for the current global shield-main script; explicit Dark Edge simulation first pass 7/9 at `/tmp/agentank-runs/simulations/2026-07-09T02-52-49-210Z`, then rerun 9/9 at `/tmp/agentank-runs/simulations/2026-07-09T02-53-58-638Z`.
  - Publish: Dark Edge codeVersion 92, codeHash `882b790203d36bdaf2448ea9b96ba2c8731fec9331434f0ca8ddebabc432c186`.
  - Live snapshot after publish: `/tmp/agentank-runs/snapshots-dark-edge-v92-publish/dark-edge/tank.json`, rankScore 1863, Champion I, public rank 109, record 8223-7310-0, codeVersion 92 confirmed. No post-publish real challenge-run was executed. Next review should use real post-v92 matches before touching KAI enemy-overload offset no-exit or Mira teleport-star tempo.
- Shield-main / tank 4839 `500` v52 hard-20 diagnostic and v53 passive-hold release publish:
  - User supplied v52 failure samples `mat_JN2AI4iCpxiJOUcfy` and `mat_0oJWo7VUjbM2uihCn`. Raw replay review confirmed two actionable symptoms: excessive runtime in low-value control states, and missed lane/shot conversion while boost tempo was active.
  - User explicitly requested a hard 20 real challenge run without stopping. Pre-run gates passed: `npm run check`, focused shield-main tests, `npm run test:lab`, `.env npm run simulate` 9/9 at `/tmp/agentank-runs/simulations/2026-07-08T09-24-17-351Z`, and dry-run `/tmp/agentank-runs/shield-main/hard20-v52/challenge-runs/2026-07-08T09-24-17-885Z.json`.
  - Real hard-20 run: `/tmp/agentank-runs/shield-main/hard20-v52/challenge-runs/2026-07-08T09-25-05-079Z.json`. Result 9W/11L, score 1145 -> 1106, net -39. Raw replays are under `/tmp/agentank-runs/shield-main/hard20-v52/raw`.
  - Cluster read: losses concentrated in `fix-shield-no-conversion` 11, `fix-star-tempo-loss` 10, `fix-lost-initiative` 8, `fix-no-pressure-loss` 6, `fix-bullet-death` 5, and `fix-post-shield-reset` 5. The specific local axis is movement cleanliness after boost: late/edge side-pressure boost was sending 500 into low-value corners, and passive grass/lead-control holds could swallow frames with speech only.
  - Bounded local patch: no-star side-pressure boost now refuses late or edge casts unless a direct gunline exists; late pressure boost also yields when the star is too far or the race is clearly lost. One-frame lane exits can use a close aimed lane escape when that is the only immediate safe move. Grass-camper, strategic grass, and lead-grass empty holds now expire after repeated stuck frames or late round time, and current grass hold requires real star-line or pressure control.
  - Regressions added: `shield-main does not cast late edge boost for side pressure without star or gunline`; `shield-main drives away along a close aimed lane when that is the one-frame escape`; `shield-main releases repeated grass camper hold instead of timing out`; `shield-main releases repeated lead grass hold when the current grass no longer controls value`.
  - Verification before publish: focused shield-main tests passed 128/128; `npm run check` passed; `npm run test:lab` passed; `git diff --check` passed; `.env npm run simulate` passed 9/9 at `/tmp/agentank-runs/simulations/2026-07-08T09-42-45-145Z`; `.env npm run challenge:dry` passed at `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-08T09-43-29-063Z.json` with live start score 1125.
  - Publish rerun gates: `npm run check` passed; `npm run test:lab` passed; `git diff --check` passed; `.env npm run simulate` passed 9/9 at `/tmp/agentank-runs/simulations/2026-07-08T09-55-31-347Z`; `.env npm run challenge:dry` passed at `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-08T09-56-10-911Z.json` with live start score 1165.
  - Publish: shield-main codeVersion 53, codeHash `ac09e1380bb8bfd8c8784713bfee3acc433411f0d42093e0144a780fed002cc8`.
  - Live snapshot after publish: `/tmp/agentank-runs/snapshots-shield-v53-publish/shield-main/tank.json`, rankScore 1165, Platinum I, record 751-721-0, codeVersion 53 confirmed. No post-publish real challenge-run was executed.
- Shield-main / tank 4839 `500` local boost initiative pressure refactor after continued live drift:
  - Trigger: user reported the tank kept dropping after the boost pivot and was too passive: "没有开过枪". Latest shield dry-run read live start score 1153 while published code remains v51.
  - Leaderboard research: ran `node lab/scripts/research-boost-leaderboard.mjs --top-tanks 15 --matches-per-tank 5 --max-matches 50`; report at `/tmp/agentank-runs/boost-leaderboard-research/2026-07-08T08-41-09-929Z/report.md`. Sampled 10 high-score boost tanks / 50 matches. Focus sample was 23W/27L, opening boost 24/50, boost before first own star 45/50, no-boost games 3/50. Wins averaged 5.52 shots vs losses 1.89; star wins averaged 11.38 shots. This supports a pressure/initiative refactor instead of more far-star micro-guards.
  - Dark Edge comparison: reusable idea is not overload-specific code, but its frame arbitration: when the star race is lost or dangerous, it converts to pressure ETA and gunline/side-lane control before blind value movement.
  - Local refactor: added L2 `boost-initiative-pressure` after `boost-confirmed-shot` and before `boost-star-tempo`. It fires or turns on clear gunlines, can cast boost to safe side pressure lanes when no close star/value route exists, yields to close safe stars, valuable boost star routes, and nearby grass star-control, and avoids late no-star side-lane scans while safely ahead.
  - Hard-constraint follow-up from simulation: first 8/9 simulation lost `azure-hunter` classic by bullet crash while leading 3-0 with 0 shots; replay showed opaque active enemy bullet state let star movement continue along the enemy gunline. `enemyLaneThreatDirectionAt()` now treats opaque active enemy bullets as preserving the enemy's current gunline, while visible bullets still use real bullet tracking. Regression added: `shield-main does not chase a star along an opaque active enemy bullet lane`.
  - Runtime-budget follow-up: second 8/9 simulation fixed `azure-hunter` classic but lost `azure-hunter` random by timeout after late no-star boost side-lane scans. Candidate now cheaply filters side-lane candidates before `pathInfo()` and disables side-lane boost scanning after frame 96 while safely ahead without a star. Regression added: `shield-main does not scan for late boost side lanes while safely ahead without a star`.
  - Regressions added in this cycle: `shield-main fires before lead far-star pressure movement`; `shield-main casts boost to take a safe side pressure lane when no star is available`; `shield-main does not scan for late boost side lanes while safely ahead without a star`; `shield-main does not chase a star along an opaque active enemy bullet lane`; strategy pipeline now requires `L2:boost-initiative-pressure`.
  - Verification and publish: focused shield-main tests passed 124/124; `npm run check` passed; `npm run test:lab` passed; explicit `.env` simulation passed 9/9 at `/tmp/agentank-runs/simulations/2026-07-08T08-57-55-020Z`; rerun before publish passed `npm run check`, focused shield-main tests 124/124, `.env` simulation 9/9 at `/tmp/agentank-runs/simulations/2026-07-08T09-10-50-516Z`, and shield-main dry-run at `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-08T09-11-29-296Z.json` with start score 1153, `stopOnLoss=true`, `drawdownStop=25`. Published shield-main codeVersion 52, codeHash `a1c420d9e4ebc67f35df146ebf1e16df43de13954eaec67e35ee15c3029a039d`.
  - Live snapshot after publish: `/tmp/agentank-runs/snapshots-shield-v52-publish/shield-main/tank.json`, rankScore 1153, Platinum I, public rank 673/4434, record 734-705-0, codeVersion 52 confirmed. No real challenge-run was executed.
- Shield-main / tank 4839 `500` v50 publish, v50 stop-loss, and local v51 recent-boost adjacent-star candidate:
  - Publish gates before v50: focused shield-main tests passed 118/118; `npm run check` passed; `npm run test:lab` passed; `git diff --check` passed; `.env` simulation passed 8/9 at `/tmp/agentank-runs/simulations/2026-07-08T07-31-37-089Z`; shield dry-run passed at `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-08T07-32-15-468Z.json`.
  - Publish: shield-main codeVersion 50, codeHash `ad7b3f255b28a0ff4cd5f8f5fd7119713dcd29797f70ba482189824d44b87552`.
  - Snapshot after publish: `/tmp/agentank-runs/snapshots-shield-v50-publish/shield-main/tank.json`, rankScore 1291, Diamond III, public rank 596, record 732-696-0. A v49 defense win `mat_CLV4q6cs2bg2gninx` had restored score 1270 -> 1291 before the v50 run.
  - Real challenge: user requested real challenge-driven iteration. v50 run `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-08T07-33-49-301Z.json` stopped after first loss by stopOnLoss. Result 0W/1L, score 1291 -> 1272. Loss `mat_GBmkPh3FQdy63G7I1` vs poison tank `Sd.Kfz. 182`, random map, 0-4 by stars.
  - Replay diagnosis: 500 cast boost 4 times, collected 0 stars, fired 1 shot into a wall. On the opening star `[9,7]`, 500 boosted from `[2,2]` to the star column and reached `[9,6]` by frame 10 with the star one step below, but recent-boost pathing replanned away from the adjacent star under future shot pressure. The opponent collected at frame 17. Later boost casts also failed to convert, but the first lost adjacent-star window is the single bounded fix axis.
  - Bounded local patch: `tryBoostStarLandingGuard()` now handles recent-boost adjacent stars before boost ETA/path replanning. It collects after boost expiry unless a real bullet, own bomb, overload, hidden shooter, hard block, or point-blank enemy pickup covers the star; active boosted one-step pass-through hold remains protected.
  - Regressions added: `shield-main collects an adjacent star after recent boost instead of replanning away`; `shield-main does not force a recent-boost adjacent star through an active bullet`.
  - v51 gates: focused shield-main tests passed 120/120; `npm run test:lab` passed; `npm run check` passed; `git diff --check` passed; `.env` simulation passed 9/9 at `/tmp/agentank-runs/simulations/2026-07-08T07-44-46-421Z`; post-publish shield dry-run passed at `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-08T07-46-14-578Z.json`.
  - Publish: shield-main codeVersion 51, codeHash `671068cd5ced10f25270218190f0b517ccb86c2d227365ee8d01a8b57f70408c`.
  - Live snapshot after publish: `/tmp/agentank-runs/snapshots-shield-v51-publish/shield-main/tank.json`, rankScore 1272, Diamond III, public rank 607, record 732-697-0, codeVersion 51 confirmed.
  - v51 real challenge: `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-08T07-46-22-402Z.json` stopped after first loss by stopOnLoss. Result 0W/1L, score 1272 -> 1252. Loss `mat_D7rGhn6agqCLyer30` vs boost tank `Overlord`, random map, 2-4 by stars.
  - v51 replay read: the recent-boost adjacent-star fix held, with 500 collecting `[14,4]` at frame 51 after boost expiry. The new loss shape is boost-mirror far-star tempo: Overlord cast boost at frames 4, 45, 72, and 107, collecting later far stars at frames 76, 96, and 117 while 500 trailed. Treat as a cluster/research axis before any new code patch, not a single-loss patch.
  - Snapshot after v51 real loss: `/tmp/agentank-runs/snapshots-shield-v51-after-real1/shield-main/tank.json`, codeVersion 51, rankScore 1252, public rank 620, record 732-698-0. Stop further real challenges until this new boost-mirror far-star tempo cluster is reviewed.
- Shield-main / tank 4839 `500` v49 real stop-loss and local late reachable star candidate:
  - Gates before real challenge: `npm run check` passed; focused shield-main tests passed 117/117 before the patch; `npm run test:lab` passed; `.env` simulation passed 9/9 at `/tmp/agentank-runs/simulations/2026-07-08T07-18-02-588Z`; shield dry-run passed at `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-08T07-18-03-461Z.json`, start score 1290, stopOnLoss true.
  - Real challenge: user requested real challenge-driven training. Run `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-08T07-19-01-020Z.json` stopped after the first loss by stopOnLoss. Result 0W/1L, score 1290 -> 1270. Loss `mat_4mHSpmGsHL3IaFeq4` vs teleport tank Moming, random map, 2-3 by stars.
  - Replay diagnosis: 500 used boost 3 times, collected 2 stars, fired 5 shots, and all 5 shots hit walls. Opening boost still lost to teleport's frame-1 star pickup. The clearest bounded fix is the late star `[13,9]`: after boosting to the star column, 500 reached `[13,6]` by frame 114 and then fired down the star line at frames 116, 119, 122, and 125, all into walls, instead of walking three safe cells to the reachable star.
  - Bounded local patch: added L2 `late-reachable-star` before `star-tempo-arbiter`, `star-interception`, and `star-lane-pressure`. It only takes short safe late stars when the enemy route is not clearly faster, preserving hard danger, clearly lost teleport star-line fire, and existing boost guards.
  - Regression added: `shield-main takes a late reachable star before wall-only fire`; existing `shield-main fires down a lost teleport star lane instead of walking into the pickup` remains the preserve boundary.
  - Snapshot after loss: `/tmp/agentank-runs/snapshots-shield-v49-after-real1/shield-main/tank.json`, codeVersion 49, rankScore 1270, record 731-696-0. Candidate is local only; no publish yet.
- Shield-main / tank 4839 `500` method reset toward baseline recovery audit:
  - User confirmed the project should continue with the `agentank-evolution-lab`-derived method already recorded in `docs/agentank-evolution-method.md`, `state/training-space.json`, and `state/cycle.md`.
  - Local evidence supports pausing v49 micro-patches: current live state is codeVersion 49 at rankScore 1290, while the boost chain had healthier preserve evidence around v35-v42 and then drifted down through v44-v49.
  - This is not a code patch and should not trigger publish. The next training round should first audit the baseline chain and real replay clusters, then make one explicit decision: rollback candidate, consolidation patch, or one bounded refactor.
  - Do not continue adding isolated guards to v49 until the recovery audit identifies the next single axis. Real challenges still require explicit confirmation and stop-loss.
- Dark Edge v91 reachable-hidden-shooter side-exit publish:
  - Reviewed user-provided v90 matches `mat_60urM60h84iBrNYtJ`, `mat_DoQ8YbZNZY1FfgOAy`, `mat_Dyki3nYY9R07nKecL`, and `mat_9Qe2E9pJ9V1EDPttQ`. All Dark Edge samples used codeHash `0eb582e1f80d9ff081a0b6951e62dc21ae6de7aab77376c9e5a7700d55d83355`.
  - `mat_60urM60h84iBrNYtJ` is the direct fix sample: after the defender cast cloak at frame 17, Dark Edge sat at `[6,12]` and turned in place through frames 18-23. The defender moved to `[8,12]` while cloaked, fired left at frame 23, and killed Dark Edge. This was not covered by v90's visible-bullet fixed-line memory because no bullet existed before the final shot.
  - `mat_Dyki3nYY9R07nKecL` is the adjacent movement-cleanliness sample: after leading 2-0, Dark Edge reached the left edge at `[1,3]` and spun from frames 30-36 while the defender held grass/line pressure around `[6,3]`, then died to a left shot. Treat this as the same reachable-hidden-shooter exit axis, not as a star-tempo patch.
  - `mat_DoQ8YbZNZY1FfgOAy` explains the opponent style: teleport tank `Xenon_I` used teleport for star tempo at frames 1, 66, and 110, collected 4 stars, fired 0 shots, and spent 47/128 frames in grass positions. This is a teleport-star-tempo + grass concealment matchup, not part of this patch axis.
  - `mat_9Qe2E9pJ9V1EDPttQ` is a preserve sample: Dark Edge won 3-2 by stars, cast overload three times, and fired only when an active overload offset line existed. The apparent no-fire behavior during mutual overload was mostly because both tanks avoided clean offset conversion windows; keep it as a watch item, not a patch trigger.
  - Bounded local patch: predicted hidden-shooter lines now expose a direction, and `tryPredictedHiddenShooterExit()` runs inside L0 emergency defense after visible-bullet fixed-line exit and before generic hidden-lane dodge. It uses a safe side exit that keeps real bullet, aimed-shot, breakable-lane, overload, wall, bomb, and hidden straight-line pressure checks, but avoids letting speculative hidden-shooter scoring reject every exit and swallow the frame.
  - Regressions added: `dark-edge exits a reachable cloaked shooter row instead of spinning`; `dark-edge exits a predicted grass shooter row instead of edge spinning`.
  - Pre-publish caution: the first explicit server simulation completed 7/9 at `/tmp/agentank-runs/simulations/2026-07-08T07-12-38-500Z`; the two losses were runtime/no-value loops with 0 stars and 0 shots against `Nova Scout` random and `Azure Hunter` classic. User accepted stabilizing this candidate after method review.
  - Publish gates before v91: focused Dark Edge tests passed 53/53; `npm run check` passed; `git diff --check` passed; standard Dark Edge server simulation passed 9/9 at `/tmp/agentank-runs/simulations/2026-07-08T07-25-14-734Z`; Dark Edge dry-run passed at `/tmp/agentank-runs/dark-edge/challenge-runs/2026-07-08T07-25-58-444Z.json` with start score 2360. Full `npm run test:lab` is currently blocked by unrelated dirty shield-main behavior: `shield-main takes a late reachable star before wall-only fire` returned `fire` instead of expected `go`.
  - Publish: Dark Edge codeVersion 91, codeHash `f202de6b9c9b60c263404df404979d70afc3d2af2435bfe1a1e7074e78b5500f`.
  - Live snapshot after publish: `/tmp/agentank-runs/snapshots-dark-edge-v91-publish/dark-edge/tank.json`, rankScore 2360, Champion I, public rank 91/4430, record 8041-7195-0, codeVersion 91 confirmed. No real challenge-run was executed.
- Shield-main / tank 4839 `500` v48 target-10 stop-loss and v49 opening folded boost race guard publish:
  - User samples: reviewed v48 `mat_5mYcx5n1trzAsX2kH` and `mat_4zrJb06O5NaIOzBHM`. The first was poison, not stun; the second was stun and confirmed a bad shape after control: 500 collected a star while stunned/reversed, tried to leave the lane, then reversed back into the bullet. Both still had low pressure creation, but they were not enough alone for a new stun patch.
  - Real challenge: user confirmed a target-10 real run. The existing stop-loss policy stopped after 2 matches at `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-08T06-51-28-545Z.json`: win `mat_3HmzfWTKgAaDfAzWV` +20 by stars, then loss `mat_8cRYhZMqS1l8IduOi` -20 by stars, score 1290 -> 1310 -> 1290.
  - Replay diagnosis: the loss was not a stun case. Opening star `[11,5]` made 500 cast boost at frame 1, then the long first boosted segment swept across row 2 before turning down. The non-mobile opponent used a more direct star line and collected at frame 14; the frame-25 mutual kill still lost because the opponent already had the star.
  - Bounded patch: `boostStarWorthwhile()` now rejects only early long folded opening boost routes against non-boost/non-teleport opponents when the opponent has a comparable direct star race. This preserves mobile-opponent opening boost, valuable folded routes, wall-capped route rejection, late boost guards, confirmed shots, stun-pin protection, and hard danger.
  - Regression added: `shield-main does not boost an opening folded route against a direct non-mobile star race`.
  - Verification: focused shield-main tests passed 117/117; `npm run check` passed; `npm run test:lab` passed; `git diff --check` passed; JSON state parse passed; explicit `.env` server simulation passed 9/9 at `/tmp/agentank-runs/simulations/2026-07-08T06-57-11-830Z`; `.env` shield dry-run passed at `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-08T06-57-56-534Z.json`, start score 1290, stop-on-loss policy active.
  - Publish: shield-main codeVersion 49, codeHash `49bcf6a75757233166e449660fab587cdf600a12c67d050fdbf839dd028aae96`.
  - Live snapshot after publish: `/tmp/agentank-runs/snapshots-shield-v49-publish/shield-main/tank.json`, tank 4839 `500`, skillType boost, rankScore 1290, Diamond III, public rank 606, record 731-695-0, codeVersion 49 confirmed. No post-v49 real challenge was executed.
- Shield-main / tank 4839 `500` local boost confirmed-shot priority candidate:
  - Trigger: user noted the current boost strategy is not good and appears to keep walking downhill even if the code is not syntactically conflicting. Live refresh at `/tmp/agentank-runs/snapshots-shield-current-structural-review/shield-main/tank.json` still showed codeVersion 47, rankScore 1295, Diamond III, record 727-691-0; no new post-v47 real match sample was visible.
  - Code-quality diagnosis: the current boost line still lets far-star, grass, and star-control value modules consume frames before ordinary L3 clear-fire. This is not a syntax conflict, but it is a priority conflict for boost play: high-score boost tanks convert acceleration into pressure, while this line can still drift toward far-star value when an already-aimed clear shot exists.
  - Bounded patch: added L2 `boost-confirmed-shot` before boost star-tempo/control. It only applies to `skill.type === "boost"`, no current hard danger, no current bullet or own-bomb danger, no enemy shield, and a clear gunline. It explicitly yields to safe adjacent stars and safe near-star pickups.
  - Regressions added: `shield-main boost confirmed shot fires before far-star value drift`; `shield-main still takes an adjacent safe star before boost confirmed fire`. Strategy pipeline tests now require `L2:boost-confirmed-shot`.
  - Verification: focused shield-main tests passed 116/116; `npm run check` passed; `npm run test:lab` passed; `git diff --check` passed; JSON state parse passed; explicit `.env` server simulation passed 9/9 at `/tmp/agentank-runs/simulations/2026-07-08T06-34-14-510Z`; `.env` shield dry-run passed at `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-08T06-35-53-498Z.json`, start score 1295, stop-on-loss policy active.
  - Publish: shield-main codeVersion 48, codeHash `4702bd79d37ca00be84d19dec4804c8c285dbe2a908c2ea72b414e886244dd34`.
  - Live snapshot after publish: `/tmp/agentank-runs/snapshots-shield-v48-publish/shield-main/tank.json`, tank 4839 `500`, skillType boost, rankScore 1295, Diamond III, public rank 602, record 727-691-0, codeVersion 48 confirmed. No post-v48 real challenge was executed.
- Shield-main / tank 4839 `500` v46-v47 regression review and late boost conversion publish:
  - Trigger: user reported that after switching to boost the tank felt increasingly worse and suspected old shield-era strategy conflicts. Reviewed the latest v44-v46 visible-match cluster instead of patching from one emotional sample.
  - Replay cluster: raw replays stored under `/tmp/agentank-runs/shield-v44-v46-regression-cluster`. Runtime losses repeatedly spent boost too late to convert: `mat_1jBM2C8EjPM0A9Q6v` and `mat_4rwgxJG0HlHI7kH2P` had defender 500 late boost casts at frame 124; `mat_1c3IabZYghYDGX3uv` had a runtime loss with a boost cast at frame 127. Preserve contrast: winning `mat_KwFwU0f9w6Q8LKsjR` converted 4-1 stars and did not need a late boost after frame 85.
  - Bounded patch: after frame 118, `boostStarWorthwhile()` and boost star-control casting only allow boost for the current star when the boosted ETA can still reach it before frame 127. This does not change early boost routing, stable near-star saving, wall-capped route rejection, stun-pin protection, grass control, or pressure conversion.
  - Regressions added: `shield-main does not cast late boost for an unreachable final star`; `shield-main can still cast late boost when the final star is reachable`.
  - Verification: focused shield-main tests passed 114/114; `npm run check` passed; `npm run test:lab` passed; `.env` server simulation passed 9/9 at `/tmp/agentank-runs/simulations/2026-07-08T06-13-53-460Z`; `.env` shield dry-run passed at `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-08T06-14-32-135Z.json`, start score 1295, stop-on-loss policy active.
  - Publish: shield-main codeVersion 47, codeHash `51a644a68ac4be4479522ef9c316ffd03086f68759e8de4e72a9eafef923c65f`.
  - Live snapshot after publish: `/tmp/agentank-runs/snapshots-shield-v47-publish/shield-main/tank.json`, tank 4839 `500`, skillType boost, rankScore 1295, Diamond III, public rank 600, record 727-691-0, codeVersion 47 confirmed. No post-v47 real challenge was executed.
- Shield-main / tank 4839 `500` v45 post-publish target-20 stop-loss and v46 stun-controlled star-pin publish:
  - Real challenge target 20 after v45 publish used the existing adaptive runner with stop-on-loss and stopped after the first match: `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-08T05-59-20-154Z.json`. Result 0W/1L, score 1336 -> 1316. Loss `mat_446Bx2KBrxs91nOxi` vs stun tank `mm`, crash loss, 0-1 stars.
  - Replay diagnosis: 500 was stunned/reversed near star `[11,5]`. At frame 18 it reverse-walked from `[11,3]` to `[11,4]` while `mm` moved onto the star at `[11,5]`; by frame 20 `mm` fired upward from point-blank range and killed 500. The v45 wall-capped boost guard was not involved.
  - Bounded patch: strict `safeCell()` now rejects a stun-controlled star pin only when 500 is stunned or reversed, the opponent is a stun/cloak skill trap, both tanks are next to the contested star, and the candidate value move would step onto the star line beside the opponent's pickup. This preserves ordinary safe adjacent stars, real one-frame lane exits, useful folded boost routes, grass star-control, and pressure conversion.
  - Regression added: `shield-main does not reverse-walk into a stun-controlled star pin`.
  - Verification: focused shield-main tests passed 112/112; `npm run check` passed; `npm run test:lab` passed; `.env` server simulation passed 9/9 at `/tmp/agentank-runs/simulations/2026-07-08T06-03-23-105Z`; `.env` shield dry-run passed at `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-08T06-03-23-723Z.json`, start score 1316, stop-on-loss policy active.
  - Publish: shield-main codeVersion 46, codeHash `1a8de8641f0c0066ae9d37f3c958ee951eb3d4f2ce57dfbfbda18c5de5c710a3`.
  - Live snapshot after publish: `/tmp/agentank-runs/snapshots-shield-v46-publish/shield-main/tank.json`, tank 4839 `500`, skillType boost, rankScore 1316, Diamond II, public rank 574, record 727-690-0, codeVersion 46 confirmed. No post-v46 real challenge was executed.
- Shield-main / tank 4839 `500` v44 real target-20 stop-loss and v45 wall-capped boost route publish:
  - Real challenge target 20 used the existing adaptive runner with stop-on-loss and stopped after the first match: `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-08T05-49-51-924Z.json`. Result 0W/1L, observed run score 1392 -> 1371. Loss `mat_KARHh2131hj1nY3rh` vs same-skill boost tank `Veylor` ended by stars, 2-3.
  - Replay diagnosis: 500 cast boost at frame 3 toward the opening star `[13,3]`, but the boosted first segment ran along row 2 into the `[12,2]` hard-wall / top-edge area before reaching the star lane. After boost expiry it had to turn near the obstacle/edge, lost the first star at frame 13, and later remained behind on star tempo. This matches the user-observed pattern of boost turning into no-purpose movement beside obstacles.
  - Bounded patch: `cleanBoostStarRoute()` now rejects wall-capped opening star routes whose long first boosted segment ends at an edge wall before reaching the star. It does not hard-veto valuable folded boost routes, does not change grass priority, and keeps hard danger, adjacent safe stars, stable closer-star boost-saving, and boost pressure conversion intact.
  - Regression added: `shield-main does not boost into a wall-capped opening star route`.
  - Verification: focused shield-main tests passed 111/111; `npm run check` passed; `npm run test:lab` passed; `.env` server simulation completed 8/9 at `/tmp/agentank-runs/simulations/2026-07-08T05-55-27-775Z` with the lone loss a random-map runtime/no-pressure case outside this patch axis; `.env` shield dry-run passed at `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-08T05-55-28-350Z.json`.
  - Publish: shield-main codeVersion 45, codeHash `d3ab0cc9f2a0593e9023d896418d9b00753c5ea201aca5a39981d5792ddd22d1`.
  - Live snapshot after publish: `/tmp/agentank-runs/snapshots-shield-v45-publish/shield-main/tank.json`, tank 4839 `500`, skillType boost, rankScore 1336, Diamond II, public rank 548, record 727-689-0, codeVersion 45 confirmed. The post-v45 real challenge is recorded in the v46 entry above.
- Dark Edge v90 fixed-shooter lane exit publish:
  - Scope: user-provided myth/grass/cloak cluster showed repeated turn-in-place after the opponent exposed a fixed shooter row/column with visible bullets. Kept this as one bounded L0 axis: fixed shooter line memory plus current-line side exit.
  - Bounded patch: `rememberHiddenShotFromBullet()` now records shooter source lines from visible enemy shots as well as hidden shots. `tryRememberedShooterExit()` runs inside emergency defense before generic hidden-lane dodging, and prefers a safe side exit when Dark Edge is already standing on the exposed line. The side-exit safety check preserves real bullet, aimed-shot, breakable-lane, overload, wall, bomb, and hidden straight-line pressure, but does not let speculative cloak reachable-position scoring block the immediate exit.
  - Regressions added: `dark-edge exits a remembered shooter column instead of turning in place`; `dark-edge does not enter a fixed shooter line exposed by a visible bullet`.
  - Verification: focused Dark Edge tests passed 51/51; `npm run check` passed; `npm run test:lab` passed; `git diff --check` passed; standard Dark Edge server simulation passed 9/9 at `/tmp/agentank-runs/simulations/2026-07-08T06-17-16-184Z`; Dark Edge dry-run passed at `/tmp/agentank-runs/dark-edge/challenge-runs/2026-07-08T06-17-50-932Z.json`, start score 2536.
  - Publish: Dark Edge codeVersion 90, codeHash `0eb582e1f80d9ff081a0b6951e62dc21ae6de7aab77376c9e5a7700d55d83355`.
  - Live snapshot after publish: `/tmp/agentank-runs/snapshots-dark-edge-v90-publish/dark-edge/tank.json`, rankScore 2518, Champion I, record 8005-7163-0, codeVersion 90 confirmed. No real challenge-run was executed.
- Shield-main / tank 4839 `500` v44 bounded grass-star-control publish:
  - Reviewed five user-provided v43 matches: `mat_5j5XpWIuvubI3pKYk`, `mat_HbJbhDTTjfQGjSuQe`, `mat_1gTBsvDQe6ELwfo49`, `mat_Gf2stkIjLWe5Ri7S9`, and `mat_8t4wSYyCHPbHG945O`. All reviewed samples used live v43 codeHash `966c986b7826657a903c25ea23f3a1e15c173fbfd3bd81be995c9308fd40df50`.
  - Replay conclusion: cloak/grass opponents gain value by turning the current star into a hidden or grass-controlled firing lane. `mat_Gf2stkIjLWe5Ri7S9` is the preserve sample: grass leverage is good only when it supports star tempo or pressure. `mat_8t4wSYyCHPbHG945O` is the clearest fix sample: 500 won only because the opponent self-bombed after controlling the star area from grass-like stations, while 500 had 0 stars and low value creation. `mat_HbJbhDTTjfQGjSuQe` still shows hidden/cloak gunline risk, but that is a separate future axis from grass priority.
  - Bounded patch: boost star-control now gives higher score to safe grass stations that control the current star line, expands contested star-line candidates only under star pressure or close enemy race, and labels those moves as `grass-control`. Existing `tryStrategicGrassControl()` is now an explicit L4 module, still bounded by hard danger and stable adjacent safe star pickup.
  - Regressions added: `shield-main favors a grass star-control station in a contested boost lane`; `shield-main still takes an adjacent safe star before grass control`. Strategy pipeline tests now require `L4:strategic-grass-control`.
  - Verification: focused shield-main tests passed 110/110; `npm run check` passed; `.env` server simulation passed 9/9 at `/tmp/agentank-runs/simulations/2026-07-08T03-56-22-590Z`; `.env` shield dry-run passed at `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-08T03-57-04-652Z.json`, start score 1393, stop-on-loss policy active. Full `npm run test:lab` and `git diff --check` passed after the publish with no code changes after codeVersion 44.
  - Publish: shield-main codeVersion 44, codeHash `1c60898502f37aee3f44ff48bb004524bb370145bebbca346eaff5f386a664c9`.
  - Live snapshot after publish: `/tmp/agentank-runs/snapshots-shield-v44-publish/shield-main/tank.json`, tank 4839 `500`, skillType boost, rankScore 1393, Diamond II, record 725-684-0, codeVersion 44 confirmed. No real challenge was executed after publish.
- Dark Edge v89 grass-control actionization publish:
  - Scope: converged the unpublished v88 hidden-shooter local candidate before release. The isolated patch axis was movement-cleanliness inside grass control: strategic/lead/grass-camper hold frames may keep grass priority only when they submit a concrete action, otherwise they fall through to later modules.
  - Root cause from server simulations: the previous local candidate reached strategic grass and then produced `say`-only frames for many ticks, causing runtime losses in otherwise non-combat random-map checks.
  - Bounded patch: added `tryActiveGrassHold()` and routed current-tile grass holds through overload, safe fire, enemy-line turn, or star-line turn. `tryGrassCamperHold()` also now requires the current tile to be pressure grass before it can claim the frame.
  - Regressions added: `dark-edge turns a held strategic grass tile into an action`; `dark-edge does not let stale grass-camper memory swallow non-grass movement`.
  - Verification: focused Dark Edge tests passed 49/49; `npm run check` passed; `npm run test:lab` passed; `git diff --check` passed; standard Dark Edge server simulation passed 9/9 at `/tmp/agentank-runs/simulations/2026-07-08T03-49-00-974Z`; Dark Edge dry-run passed at `/tmp/agentank-runs/dark-edge/challenge-runs/2026-07-08T03-49-58-477Z.json`, start score 2383.
  - Publish: Dark Edge codeVersion 89, codeHash `87a8371413b39b3bfe9e3f0d4b1f2991757bb39a2ce9b849124ad7cbc37c9515`.
  - Live snapshot after publish: `/tmp/agentank-runs/snapshots-dark-edge-v89-publish/dark-edge/tank.json`, rankScore 2365, Champion I, record 7973-7145-0, codeVersion 89 confirmed. No real challenge-run was executed after publish.
- Dark Edge v88 user replay grass/cloak review and local hidden-shot perception candidate:
  - Reviewed user-provided live v88 matches `mat_0zFe93Wu7XZ0YwjWj`, `mat_ArV2RuWC9YX4T4Vcn`, `mat_7sCKJ3bnSlV6ESXby`, `mat_D8bh3sn6a0Z2bUS2i`, and `mat_2K1c8OXA28L5wwyHn`. All reviewed Dark Edge samples used published v88 codeHash `977f5670fc20321f655d60c17b45dff44d47ab611f3d1657dbb046a13a8a3d27`.
  - Loss `mat_2K1c8OXA28L5wwyHn` exposed the same hidden-shooter hard axis as the prior local cloak candidate: VANTA cloaked from `[12,9]`, reached `[12,6]`, then killed Dark Edge on the star tile `[10,6]`. Added a replay-shaped regression so safe star pickup cannot override a reachable cloak pickup shooter lane.
  - User's bullet-line observation is valid and bounded: when the enemy is not visible but its bullet is visible, Dark Edge now remembers the exposed row/column and direction briefly as hidden-shooter pressure. This is L0 perception only; it does not increase generic grass camping priority or allow passive grass idling.
  - Preserve samples: `mat_ArV2RuWC9YX4T4Vcn`, `mat_7sCKJ3bnSlV6ESXby`, and `mat_D8bh3sn6a0Z2bUS2i` confirm existing win behavior against shield tanks: collect or pressure first, let shield expire, then convert with clear fire/overload pressure. Do not replace this with blanket early overload or passive grass holding.
  - Separate unresolved axis: `mat_0zFe93Wu7XZ0YwjWj` is a visible long bullet lane / no-exit-turning failure against teleport tempo, not a hidden/grass bullet-inference case. Keep it out of this patch.
  - Regressions added: `dark-edge refuses a star covered by a reachable cloaked pickup shooter`; `dark-edge remembers a hidden shooter lane from a visible bullet`.
  - Verification: focused Dark Edge tests passed 47/47; `npm run check` passed; `npm run test:lab` passed; `git diff --check` passed. Server simulation did not meet publish confidence: current candidate ran 7/9 twice at `/tmp/agentank-runs/simulations/2026-07-08T03-32-57-779Z` and `/tmp/agentank-runs/simulations/2026-07-08T03-34-50-108Z`. A temporary no-hidden-shot control also ran 7/9 at `/tmp/agentank-runs/simulations/2026-07-08T03-36-18-594Z`, so the new hidden-shot memory is not isolated as the regression cause, but the full candidate remains local only. No publish, dry-run, or real challenge was executed.
- Shield-main / tank 4839 `500` user-provided v42 replay review and local close skill-trap pressure candidate:
  - Reviewed four user-provided v42 matches: `mat_1e931JDndz2BUDjeg`, `mat_6AIR5sLj0hmBImuWK`, `mat_DfQpetqY4qTLe9BkW`, and `mat_7ccj9B7jkclKw9tfo`. All used live codeHash `632ed1a2c6e921f1264b8e7eef76339b0f0a3c72e4170e2fd004d45caefa8f84`.
  - Shared failure shape: boost was spent but pressure conversion was too low. `mat_DfQpetqY4qTLe9BkW` lost 1-3 to stun with 5 boost casts and 0 shots; `mat_7ccj9B7jkclKw9tfo` lost by bullet crash after a stun trap with 0 shots; `mat_6AIR5sLj0hmBImuWK` lost 2-4 with only 2 own shots, both into walls; `mat_1e931JDndz2BUDjeg` died while collecting a star after the opponent camped a star lane and created repeated fire pressure.
  - Root cause for this bounded patch: `boost` close-volley hard-danger logic was correctly protecting lanes, but it also let L0/L1 escape behavior outrank a clear close stun/cloak trap shot when 500 was already aimed or could turn faster than the opponent could aim. Exact reproduction from `mat_DfQpetqY4qTLe9BkW` frame 16 changed from dodge/turn to fire.
  - Bounded local patch: added `trySkillTrapTempoShot()` and call it inside `tryEmergencyDefense()` before escape. It only applies against stun/cloak, with no current bullet or own-bomb hard danger, no already-aimed enemy shot, a clear line of fire, and our shot frame no slower than the enemy aim frame. Point-blank equal-speed turn duels still keep the existing escape behavior.
  - Regressions added: `shield-main fires before escaping a non-aimed close stun trap`; `shield-main turns for a point-blank stun trap shot before reverse drifting`.
  - Verification: focused shield-main tests passed 108/108; `npm run check` passed; `npm run test:lab` passed; `git diff --check` passed; explicit `.env` server simulation completed 8/9 at `/tmp/agentank-runs/simulations/2026-07-08T03-26-29-230Z`. The only simulation loss was `shield-main-nova-scout-random-r1.json`, a runtime/two-tile-loop signal outside this patch axis. Explicit `.env` shield dry-run passed at `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-08T03-28-20-020Z.json`, start score 1351.
  - Publish: shield-main codeVersion 43, codeHash `966c986b7826657a903c25ea23f3a1e15c173fbfd3bd81be995c9308fd40df50`.
  - Live snapshot after publish: `/tmp/agentank-runs/snapshots-shield-v43-publish/shield-main/tank.json`, tank 4839 `500`, skillType boost, rankScore 1351, Diamond II, record 719-680-0, codeVersion 43 confirmed. No real challenge was executed after publish.
- Shield-main / tank 4839 `500` v40-v42 publish, stop-loss real validation, and bounded follow-ups:
  - v40 published the 40-match high-score boost learning axis: occupied boost star-control lanes convert into real pressure fire. Publish codeVersion 40, codeHash `c1d9afcb90ad1a6a7741aac18c151290db86cd4b95ab2a6e401078980b651e57`; snapshot `/tmp/agentank-runs/snapshots-shield-v40-publish/shield-main/tank.json` confirmed rankScore 1450, Diamond I, record 717-672-0.
  - v40 real validation target 20 stopped after first loss: `/tmp/agentank-runs/shield-v40-star-control-real20/challenge-runs/2026-07-08T02-41-09-597Z.json`, 0W/1L, score 1450 -> 1436. Loss `mat_A1L5RbI3QoaCCvfoE` vs overload `Zephyrus` was a hard-current-bullet failure: 500 had 0 stars / 0 shots, enemy collected 2 stars, cast overload at frame 49, and killed 500 at frame 53. Root cause was active enemy overload frames not being recognized when exposed through active remaining-frame state rather than near-ready cooldown/status.
  - v41 bounded follow-up: `enemyOverloadReady()` now treats `enemy.skill.activeRemainingFrames > 0` and overload effects as active hard danger. Regression added: `shield-main treats active overload remaining frames as hard offset danger`. Gates passed: focused shield-main tests 105/105, `npm run check`, `npm run test:lab`, server simulation 9/9 at `/tmp/agentank-runs/simulations/2026-07-08T02-47-35-952Z`, and shield dry-run. Publish codeVersion 41, codeHash `81f3d84878a15143d27f28d477f57b564bc1f6acbfa33e8975c62f988285713a`; snapshot `/tmp/agentank-runs/snapshots-shield-v41-publish/shield-main/tank.json` confirmed rankScore 1455, record 718-673-0.
  - v41 real validation target 20 stopped after first loss: `/tmp/agentank-runs/shield-v41-active-overload-real20/challenge-runs/2026-07-08T02-49-19-225Z.json`, 0W/1L, score 1455 -> 1441. Loss `mat_Fvh9qNVWMzR771FeM` vs stun `LEOpard-2` was a lead-preservation gunline failure: 500 led 2-0, boost-tempo pressure walked into `[6,6]` on the enemy's same long row at frame 45, enemy fired at frame 47, and 500 died at frame 50.
  - v42 bounded follow-up: boost-tempo pressure keeps the existing one-star pressure behavior, but when `scoreMargin() >= 2` it refuses enemy two-turn long lanes. Regression added: `shield-main does not convert boost tempo by entering a two-turn long lane`. Gates passed: focused shield-main tests 106/106, `npm run check`, `npm run test:lab`, server simulation 9/9 at `/tmp/agentank-runs/simulations/2026-07-08T02-52-38-994Z`, `git diff --check`, and shield dry-run. Publish codeVersion 42, codeHash `632ed1a2c6e921f1264b8e7eef76339b0f0a3c72e4170e2fd004d45caefa8f84`.
  - v42 real validation target 20 stopped after the first loss but net positive: `/tmp/agentank-runs/shield-v42-two-turn-lane-real20/challenge-runs/2026-07-08T02-54-39-442Z.json`, 1W/1L, score 1441 -> 1454. Win `mat_4uQUs0jRkscHZFh7B` vs `Zephyrus` preserved star tempo, initiative pressure, skill tempo, and grass leverage; behavior score 73/100. Loss `mat_0XDcYTmhyIj6EloWu` vs cloak `pika` was a 1-3 star loss with 2 own shots both into walls and no hard constraints; behavior score 34/100.
  - Current decision: v42 is published and locally verified; do not patch the `pika` cloak star-tempo loss from a single sample. Next round should cluster additional cloak/star-tempo losses and preserve the v42 Zephyrus win before changing code.
  - Live snapshot after v42 validation: `/tmp/agentank-runs/snapshots-shield-v42-after-real/shield-main/tank.json`, rankScore 1454, Diamond I, record 719-675-0, codeVersion 42 confirmed.
- Dark Edge v88 inferred enemy overload offset bullet publish:
  - Scope: refreshed Dark Edge v87 stability after publish, reviewed all post-v87 real matches visible in the latest feed, patched only the hazard-first perception layer, and published `active/dark-edge.js`. No real challenge-run was executed.
  - Live refresh before patch: `/tmp/agentank-runs/dark-edge-v87-stability-refresh-20260708/dark-edge/tank.json` confirmed v87, codeHash `8cb336d2d6cbde7a1d7b6e2977f23f6a5b22fc99aebab3165c68be6bc1a68411`, rankScore 2234, record 7950-7129-0.
  - Post-v87 real sample: latest 50 refresh showed 2 v87 matches after `2026-07-08T02:21:21Z`, 1W/1L. Win `mat_5WPmWObMVlNDxUz61` vs HotBeng preserved the desired overload action layer: Dark Edge had 2 stars, cast overload at frame 99, and killed with the double down-lane bullets at frame 101. Loss `mat_2UCQVtMITrpGiGzET` vs Critical Mass exposed a hard constraint: Dark Edge had 0 stars / 0 shots, cast overload at frame 87 without conversion, then moved into the opponent's overload offset bullet at frame 104 after the visible primary bullet was already in flight.
  - Bounded patch: infer the hidden overload offset bullet from a visible enemy primary bullet while the enemy overload window is active or recently active, then feed that inferred bullet into existing `bulletDangerAt`, `safeCell`, and hard-danger routing. This does not change star tempo, grass priority, overload attack geometry, or action priority order.
  - Regression added: `dark-edge infers the hidden offset bullet from an enemy overload shot`.
  - Gates: focused Dark Edge tests passed 44/44; `npm run check` passed; `npm run test:lab` passed; `git diff --check` passed; standard Dark Edge server simulation passed 9/9 at `/tmp/agentank-runs/simulations/2026-07-08T02-35-21-808Z`; Dark Edge dry-run passed at `/tmp/agentank-runs/dark-edge/challenge-runs/2026-07-08T02-35-22-747Z.json` with start score 2234 and stop-on-loss policy.
  - Publish: Dark Edge codeVersion 88, codeHash `977f5670fc20321f655d60c17b45dff44d47ab611f3d1657dbb046a13a8a3d27`.
  - Live snapshot after publish: `/tmp/agentank-runs/snapshots-dark-edge-v88-publish/dark-edge/tank.json`, rankScore 2234, Champion I 434 points, record 7950-7129-0, codeVersion 88 confirmed.
- Shield-main / tank 4839 `500` high-score boost 40-match replay review and local star-control pressure candidate:
  - Leaderboard sample: `/tmp/agentank-runs/boost-leaderboard-research/2026-07-08T02-28-15-203Z/report.md`, 9 high-score boost tanks / 40 recent matches, 23W/17L. Opening boost within 3 frames happened in 26/40, boost before first own star in 39/40, and no sampled game was no-boost.
  - Win pattern: wins were more often crash wins than star wins (`win:crashed` 16 vs `win:star` 6). The better boost tanks use acceleration to seize tempo, then convert star-line or lane-control positions into kill pressure instead of only walking to stars.
  - Loss pattern: delayed/no useful first star and no real pressure were the biggest holes. First own star delta above 60 frames was 1W/3L; 31-60 frames was 0W/2L. Shot-wall-heavy games (`wall>=75%`) were 9W/12L, while clean-shot games (`wall<40%`) were 8W/0L. Teleport samples averaged 66.5 frames to first own star and were 1W/2L.
  - Chosen single axis: do not add broader boost timing changes. Current 500 already has near-star boost saving, overshoot guards, and hard-danger guards. This candidate only changes `boost-star-control`: if 500 already occupies a star-control line and has a clear enemy gunline, fire or turn into pressure before empty holding or walking to another control tile.
  - Bounded patch: added `tryBoostStarControlPressure()` in `active/shield-main.js` and calls it inside `tryBoostStarControlPosition()` after direct star/boost/race safety gates and before candidate movement. This keeps hard danger, adjacent star pickup, and stable closer-star boost-saving protected.
  - Regression added: `shield-main converts an occupied boost star-control lane into pressure fire`.
  - Verification: focused shield-main tests passed 104/104; `npm run check` passed; `npm run test:lab` passed; `git diff --check` passed; explicit `.env` server simulation passed 9/9 at `/tmp/agentank-runs/simulations/2026-07-08T02-34-41-510Z`; `npm run challenge:dry` with `.env` passed at `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-08T02-35-34-756Z.json`, start score 1455, stop-on-loss policy active. No publish and no real challenge were executed yet.
- Shield-main / tank 4839 `500` v38 latest live drift and local close-volley hard-danger candidate:
  - Live refresh: `/tmp/agentank-runs/snapshots-shield-v38-refresh-4/shield-main/tank.json` showed codeVersion 38, codeHash `475940b506ed1edd045ae7a975f8f72fe0b4d18ea82c2907840e82bc8682cb49`, skillType boost, rankScore 1452, Diamond I, record 712-666-0. A later shield dry-run snapshot `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-08T02-20-47-435Z.json` read rankScore 1457, Diamond I, record 715-669-0.
  - Recent v38 defender replay batch: `/tmp/agentank-runs/shield-v38-latest20-20260708/raw-replay`, 20 random-map matches, defender perspective 6W/14L, avg behavior score 28.5/100. Outcome categories: 12 bullet_crash, 5 runtime, 3 star_win. Fix signals clustered at `fix-shield-no-conversion` 13, `fix-lost-initiative` 9, `fix-bullet-death` 8, `fix-no-pressure-loss` 8, `fix-post-shield-reset` 7, `fix-runtime-budget` 5. Hard constraints: `hard-current-bullet-eta` 8.
  - Event aggregation over the same 20 raw replays: 44 own boost casts, 19 own shots vs 52 opponent shots, 8 zero-shot losses, 8 late-or-no-shot losses, 8 defender bullet-crash losses, 5 runtime losses. Near-star boost waste was not the current cluster; the stronger cluster is boost/value movement failing to respect close firing and overload offset lanes before pressure or star conversion.
  - Representative crash frames: `mat_4NZpUp5Liz4BBfWYI` died on a close same-row shot at frame 18; `mat_HAjMGDHOEnLFpUZ29` died to a close overload offset lane at frame 56; `mat_3767PJmu3uXLxtgO0` died after staying in a close vertical lane at frame 84.
  - Bounded local patch: added boost-only `closeVolleyThreatDirectionAt(...)` and routed close direct firing lanes plus overload offset firing lanes into `currentHardDanger`, `projectileDangerAt`, `currentLaneThreatDirection`, and `tryEmergencyDefense`. The guard is active only when `me.skill.type === "boost"` so existing shield-mode clear-fire, shield-conversion, and point-blank duel behaviors remain protected.
  - Regressions added: `shield-main treats a close enemy firing lane as hard danger while boosted`; `shield-main dodges a close overload offset firing lane before value movement`.
  - Verification: focused shield-main tests passed 103/103; `npm run check` passed; `npm run test:lab` passed; explicit `.env` server simulation passed 9/9 at `/tmp/agentank-runs/simulations/2026-07-08T02-19-51-576Z`; `npm run challenge:dry` passed with start score 1457 and climb policy `maxWinsPerOpponent=1`, `drawdownStop=25`, `stopOnLoss=true`.
  - Publish: shield-main codeVersion 39, codeHash `02dd14fe1e6f5d190a1d12a76a16c1aa53cfd0da8d46f8e06e53965a5e20189f`.
  - Live snapshot after publish: `/tmp/agentank-runs/snapshots-shield-v39-publish/shield-main/tank.json`, tank 4839 `500`, skillType boost, rankScore 1457, Diamond I, record 715-669-0, codeVersion 39 confirmed. No `npm run challenge:run` was executed.
- Dark Edge v87 enemy-overload star-trap publish:
  - Scope: reviewed Dark Edge latest 50 real matches and published current `active/dark-edge.js` only. No real challenge was executed.
  - Live drop context: v86 published at rankScore 2544 on 2026-07-06; refreshed snapshot on 2026-07-08 showed Dark Edge around 2203-2223, codeVersion 86 before this patch.
  - Latest-50 review: `/tmp/agentank-runs/dark-edge-latest50-20260708`; raw summaries in `raw/`, event replays in `events/`, exact trap frames in `frames/`, aggregate report in `dark-edge-50-analysis.json`.
  - Result shape: 22 wins / 28 losses, net -134 over the 50 visible matches. Losses were 13 crashed, 11 star, and 4 runTime. Loss clustering was highly concentrated: Tank-J492KL caused 14 losses and Tracker caused 8 losses; opponent skill clustering was mostly overload.
  - Preserve from wins: 18/22 wins were crash wins. Dark Edge's strong pattern is confirmed overload/offset-line kill conversion, especially against opponents that stay in a pursuit or turn lane after star pressure.
  - Fix axis: adjacent or near-star pickups inside enemy overload offset traps. Repeated failures collected or chased a star while the opponent's overload offset bullet already covered the star tile or immediate exit; many losses had 0 skill / 0 shots before death.
  - Bounded patch: `enemyOverloadStarTrap()` now removes those stars from safe pickup, and `tryEnemyOverloadStarTrapCounter()` converts the frame to pressure or a non-trap exit.
  - Gates: focused Dark Edge tests passed 43/43; `npm run check` passed; `npm run test:lab` passed; `git diff --check` passed; standard Dark Edge server simulation passed 9/9 at `/tmp/agentank-runs/simulations/2026-07-08T02-19-51-400Z`; Dark Edge dry-run passed at `/tmp/agentank-runs/dark-edge/challenge-runs/2026-07-08T02-20-53-454Z.json` with start score 2223. Targeted simulation against real ids 4786 and 5201 is unsupported by the simulate API (`training bot not found`).
  - Publish: Dark Edge codeVersion 87, codeHash `8cb336d2d6cbde7a1d7b6e2977f23f6a5b22fc99aebab3165c68be6bc1a68411`.
  - Live snapshot after publish: `/tmp/agentank-runs/snapshots-dark-edge-v87-publish/dark-edge/tank.json`, rankScore 2223, Champion I 423 points, record 7949-7128-0, codeVersion 87 confirmed.
- Shield-main / tank 4839 `500` v38 first post-publish real validation win:
  - Live refresh: `/tmp/agentank-runs/snapshots-shield-v38-refresh-2/shield-main/tank.json` confirms codeVersion 38, codeHash `475940b506ed1edd045ae7a975f8f72fe0b4d18ea82c2907840e82bc8682cb49`, skillType boost, rankScore 1582, Master III, record 703-639-0.
  - Match: `mat_1w5GFy4dH5FA6XILa`, created `2026-07-07T14:42:04Z`, random map, challenger `staryes` vs defender `500`, v38 defender codeHash matched, result defender `500` won by stars 4-1, rankScore 1562 -> 1582.
  - Replay files: summary `/tmp/agentank-runs/shield-v38-postpublish-real/mat_1w5GFy4dH5FA6XILa.json`, raw `/tmp/agentank-runs/shield-v38-postpublish-real/mat_1w5GFy4dH5FA6XILa.raw.json`, events `/tmp/agentank-runs/shield-v38-postpublish-real/mat_1w5GFy4dH5FA6XILa.events.json`.
  - Defender-perspective review: behavior score 69/100. Preserve signals: `preserve-star-tempo-win`, `preserve-initiative-pressure`, `preserve-skill-tempo`. Brave baseline: `brave-safe-star`, `brave-initiative-pressure`. No Fix signals and no Hard Constraints for 500.
  - Interpretation: treat this as a positive v38 preserve sample, not a new patch trigger. It supports the current axis: acceleration converted into star tempo and initiative pressure without observed hard-risk regression.
  - Dry-run after the win: `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-07T14-44-14-261Z.json` passed with start score 1582 and climb policy `maxWinsPerOpponent=1`, `drawdownStop=25`, `stopOnLoss=true`.
- Shield-main / tank 4839 `500` v38 post-publish refresh before real validation:
  - Live refresh: `/tmp/agentank-runs/snapshots-shield-v38-refresh/shield-main/tank.json` confirms codeVersion 38, codeHash `475940b506ed1edd045ae7a975f8f72fe0b4d18ea82c2907840e82bc8682cb49`, skillType boost, rankScore 1562, Master III, record 702-639-0.
  - Recent matches refresh: `/tmp/agentank-runs/snapshots-shield-v38-refresh/shield-main/matches.json` returned 20 matches, all before the v38 publish timestamp `2026-07-07T14:37:47Z`; there is no post-publish real challenge sample to review yet.
  - Dry-run: `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-07T14-40-49-492Z.json` passed with start score 1562 and climb policy `maxWinsPerOpponent=1`, `drawdownStop=25`, `stopOnLoss=true`. The target pool currently falls back to one server-selected rank-eligible random opponent on random map.
  - Decision: do not infer v38 stability from simulation alone. The next meaningful step is an explicitly confirmed small real validation batch with the dry-run stop-loss policy, then immediate replay review of every loss/runtime/no-pressure signal.
- Shield-main / tank 4839 `500` local late boost-tempo pressure candidate:
  - Evidence: refreshed high-score boost replay sample from `/tmp/agentank-runs/boost-leaderboard-research/2026-07-07T11-12-22-429Z/report.md` still shows top boost play is not pure star walking: 48 sampled matches had many crash/star outcomes, no runtime losses in the sample summary, and high-score boost tanks frequently used acceleration before converting tempo.
  - Contrast with own v36/v37 observation: the recent 10-match real sample still had repeated `fix-no-pressure-loss`, `fix-no-value-created`, and 2 runtime losses. v37 fixed closer/near-star boost waste, but did not extend the boost-tempo pressure window after the early 72-frame cap.
  - Bounded local patch: keep v37 boost-saving rules unchanged, but extend recent `boost-tempo-pressure` lane seeking from frame 72 to frame 112. Ordinary early-lane pressure remains capped at frame 36, and safe nearby star pickup still blocks boost-tempo pressure.
  - Follow-up within the same axis: the first server simulation was 7/9 at `/tmp/agentank-runs/simulations/2026-07-07T11-18-51-814Z`; the `nova-scout` random runtime loss showed frame-50 boost followed by stale unsafe star-line hold at `[4,10]` with 0 shots. Recent boost tempo now releases stale unsafe star-line holds instead of swallowing every frame.
  - Second follow-up within the same axis: after a complete 8/9 simulation at `/tmp/agentank-runs/simulations/2026-07-07T14-32-53-854Z`, the only loss was `azure-hunter` random by bullet crash. Frame review showed boost-tempo/fallback movement entered an enemy long lane that was already aimed; boost-tempo pressure, low-value reposition, patrol, and non-panic dodge now avoid such already-aimed long lanes.
  - Publish note: the first publish attempt failed server code validation with `script timeout at tank.js:1:1`; the stale-hold release check was then made lighter by avoiding `boostTempoPressureWindow()`'s hard-danger scan inside contested star-line hold.
  - Regressions added: `shield-main converts late boost tempo into pressure before runtime drift`, covering frame 84 after a recent boost when the star is far and a safe pressure lane exists; `shield-main releases stale boost-tempo star-line hold instead of empty hold`, covering the no-shot stale star-line hold shape; `shield-main does not convert boost tempo by entering an already aimed long lane`, covering the `azure-hunter` random crash shape.
  - Verification: `npm run check` passed; `git diff --check` passed; focused shield-main tests passed 101/101; `npm run test:lab` passed; explicit `.env` server simulation passed 9/9 at `/tmp/agentank-runs/simulations/2026-07-07T14-37-03-588Z`; shield-main dry-run passed at `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-07T14-37-41-983Z.json` with start score 1562 and stop-on-loss policy.
  - Publish: shield-main codeVersion 38, codeHash `475940b506ed1edd045ae7a975f8f72fe0b4d18ea82c2907840e82bc8682cb49`.
  - Live snapshot after publish: `/tmp/agentank-runs/snapshots-shield-v38-publish/shield-main/tank.json`, tank 4839 `500`, skillType boost, rankScore 1562, Master III, record 702-639-0, codeVersion 38 confirmed. No post-publish real challenge was executed.
- Shield-main / tank 4839 `500` v37 boost timing publish and 2600 stable-goal start:
  - Goal: train shield-main / 500 / boost into a stable 2600-point phase. Stable means not a one-off touch; after reaching the 2600 area, small real challenge batches must hold the score. After that point, stop broad strategy rewrites and limit work to replay review, maintenance, and necessary small patches.
  - Method constraints: keep using `docs/agentank-evolution-method.md`, `state/training-space.json`, and this cycle file. Each round must use one explicit strategy axis, avoid single-match emotional patches, reference real high-score boost replay behavior, and pass check/test/simulate/dry-run before publish. Real challenges must use stop-loss; above 2400, stop after one loss or meaningful drawdown and review.
  - Pre-publish gates for the current boost timing candidate: `npm run check` passed; focused shield-main tests passed 98/98; `npm run test:lab` passed; `git diff --check` passed; explicit `.env` server simulation passed 9/9 at `/tmp/agentank-runs/simulations/2026-07-07T11-07-41-728Z`; `npm run challenge:dry` passed at `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-07T11-08-24-213Z.json` with climb policy `maxWinsPerOpponent=1`, `drawdownStop=25`, `stopOnLoss=true`.
  - Publish: shield-main codeVersion 37, codeHash `3e49a54a28ae680eb0d2a2581a51c758ddab4f12cd0d19987a35b8a9d79be87b`.
  - Live snapshot after publish: `/tmp/agentank-runs/snapshots-shield-v37-publish/shield-main/tank.json`, tank 4839 `500`, skillType boost, rankScore 1662, Master II, record 698-630-0, codeVersion 37 confirmed. No post-publish real challenge was executed.
- Shield-main / tank 4839 `500` v36 same-skill boost timing observation and local candidate:
  - User-provided same-skill sample: `mat_Ada8FMFyI6q0xaGMo`, v36 codeHash `0b2417501e69eb804e889e371e1f9d6d521c3bf3e8d3dd2100a20e3943bc62e1`, random map, loss by stars to boost tank `Tank 21486`, 1-5. 500 used boost 3 times and fired 0 shots; opponent used boost 4 times. The late star at frame 116 spawned near 500 (`[13,6]`, 500 rough distance 2 vs opponent 3), but 500 had just moved away and then used slow star-control turns, losing the pickup at frame 119.
  - Real 10-match observation: `/tmp/agentank-runs/shield-v36-boost-mirror-observe-10/challenge-runs/2026-07-07T10-54-42-788Z.json`; result 7W/3L, score 1623 -> 1680. Batch review average behavior score 42.6/100 with loss signals `fix-no-pressure-loss` 3, `fix-no-value-created` 3, and `fix-runtime-budget` 2.
  - Event replay aggregation over the 10 real matches: 27 own boost casts. Waste pattern confirmed: 7 casts while the opponent was already clearly closer to the star, 3 casts while 500 was already clearly closer, and 1 near-star cast at distance 2. The only real boost mirror in the batch, `mat_DiY4rWugA6uDYTDoj`, lost 2-5; at frame 52 star `[13,8]` spawned with 500 closer (rough distance 5 vs opponent 8), 500 cast boost at frame 53, then the boosted route overshot sideways and opponent collected at frame 60.
  - Bounded local patch: tighten boost timing without removing true lost-race control. `boostStarWorthwhile()` now refuses boost while already leading on a stable closer/near-star route, and `tryBoostStarControlPosition()` refuses boost-control on stable closer/near-star routes or short misaligned control boosts. The v36 behavior that spends boost for a true lost-race star-control position remains protected.
  - Regressions added: `does not boost-control a stable closer star route` and `does not boost-control a near star while already closer`.
  - Verification: focused shield-main tests passed 98/98; `npm run test:lab` passed; `npm run check` passed; explicit `.env` server simulation passed 9/9 at `/tmp/agentank-runs/simulations/2026-07-07T11-03-00-179Z`; shield-main dry-run passed at `/tmp/agentank-runs/shield-v36-boost-timing-candidate/challenge-runs/2026-07-07T11-03-45-262Z.json`, reading live rankScore 1701, Master I, record 698-628-0. Candidate is local only; no publish and no post-candidate real challenge were executed.
- Shield-main / tank 4839 `500` v35 real sample and boost-control local candidate:
  - User-provided sample: `mat_0jBocpBpVtx3WU9FG` used published codeHash `36cf31496fbe178fa59d8df74a216b6af78afef269cc288a85548a4df56d1b74` and lost 3-3 by runTime to teleport tank `eva-3`. 500 used boost 4 times and collected 3 stars, but shots were only 2/0 hit/2 wall; opening boost at frame 1 did not beat teleport to the first star, and later boosts mostly extended star walking instead of creating a durable pressure or control position.
  - Real challenge target 20 after v35 publish: `/tmp/agentank-runs/shield-v35-boost-position-20/challenge-runs/2026-07-07T10-01-33-451Z.json`; climb-policy stopped after first loss. Result 1W/1L, score 1604 -> 1603. Win: `mat_F3MekPWanqDAGFXQk` vs `phantom007`, crash win +20. Loss: `mat_5l9UHADEVEHGTFgb1` vs stun tank `潜水员`, star loss -21.
  - Loss shape: 500 used boost 4 times and collected 2 stars, but fired 0 shots. The first boost was cast at frame 2 while enemy stun/reverse control was active, causing wrong-way movement and no first-star conversion. Later boosts got one star at frame 42 and one at frame 92, but the frame 62 boost chased the third star after the stun opponent was positioned to win it; no pressure lane was created.
  - Bounded local patch: `castBoost()` now refuses boost while self stun/reverse control is active. When a direct star race is already lost, `boost-star-control` may cast boost for a safe star-control or pressure position instead of only walking there or letting `boost-star-tempo` keep chasing the lost star.
  - Regressions added: do not cast boost while stun control reverses movement; spend boost to reach a star-control position when the direct race is lost.
  - Verification: focused shield-main tests passed 96/96; `npm run check` passed; `npm run test:lab` passed; explicit `.env` server simulation passed 9/9 at `/tmp/agentank-runs/simulations/2026-07-07T10-05-38-429Z`; shield-main 20-target dry-run passed at `/tmp/agentank-runs/shield-v36-boost-control-20/challenge-runs/2026-07-07T10-06-30-407Z.json` with start score 1583.
  - Publish: shield-main codeVersion 36, codeHash `0b2417501e69eb804e889e371e1f9d6d521c3bf3e8d3dd2100a20e3943bc62e1`. Live snapshot after publish: `/tmp/agentank-runs/snapshots-shield-v36-publish/shield-main/tank.json`, tank 4839 `500`, skillType boost, rankScore 1583, Master III, record 685-621-0, codeVersion 36 confirmed. No post-publish real challenge was executed.
- Boost leaderboard reverse-engineering and local boost-tempo pressure candidate:
  - Added `lab/scripts/research-boost-leaderboard.mjs`, a read-only public-data crawler for `/api/leaderboard?period=all&sort=score&skill=boost`, tank match lists, and `view=events` replay samples. Raw public samples stay under `/tmp`, not the repo.
  - Sample run: `/tmp/agentank-runs/boost-leaderboard-research/2026-07-07T09-23-53-005Z/report.md`.
  - Sample shape: 10 high-score boost tanks, 40 recent matches, focus-tank sample record 25W/15L. Top boost play was not pure star-chase: 19/25 wins were crash wins, 6/25 were star wins, 35/40 games used boost before the first own star pickup, and only 3/40 had no boost.
  - Own v33 failure contrast: `/tmp/agentank-runs/shield-v33-star-sample-20/raw-replay` had 0W/4L, only 3 total boost casts, 2 no-boost games, and only 2 total shots. The gap is tempo conversion: high-score boost tanks use boost to seize initiative, then create pressure; 500 often kept walking or got controlled without pressure.
  - Bounded patch: added L2 `boost-tempo-pressure` so a boost tank with recent boost tempo or star lead can move into an early firing lane before ordinary far-star walking, while preserving safe nearby star pickup and hard-danger priority. First simulation was 7/9; review showed an `azure-hunter` random timeout where 500 led 2-0 but held grass with 0 shots, so lead grass control now fires or turns from a safe pressure lane before empty holding.
  - Follow-up patch: a second simulation improved to 8/9 but still lost `crimson-bastion` classic by runtime while leading 2-0 with 0 shots. Lead grass now searches a short safe pressure-lane move and, if no line is available after a stale hold, releases to later pressure/value pathing instead of swallowing the frame forever.
  - Verification after follow-up: focused `node --test lab/scripts/tests/shield-main-strategy.test.mjs` passed 94/94; `npm run check` passed; `npm run test:lab` passed; explicit `.env` server simulation passed 9/9 at `/tmp/agentank-runs/simulations/2026-07-07T09-41-19-835Z`; `npm run challenge:dry` passed at `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-07T09-41-58-507Z.json` with start score 1601. No real challenge was executed.
  - Publish: shield-main codeVersion 35, codeHash `36cf31496fbe178fa59d8df74a216b6af78afef269cc288a85548a4df56d1b74`. Live snapshot after publish: `/tmp/agentank-runs/snapshots-shield-v35-publish/shield-main/tank.json`, tank 4839 `500`, skillType boost, rankScore 1601, Master II, record 681-616-0, codeVersion 35 confirmed. No real challenge was executed.
- Shield-main / tank 4839 `500` v33 boost-route-planner publish, failed real sample, and rollback:
  - Gates before publish: focused shield-main tests passed 89/89; `npm run check` passed; `npm run test:lab` passed; explicit `.env` server simulation completed 7/9 with 2 losses at `/tmp/agentank-runs/simulations/2026-07-07T08-58-30-689Z`; explicit shield-main challenge dry-run passed at `/tmp/agentank-runs/shield-main/challenge-runs/2026-07-07T08-59-36-820Z.json` with start score 1745.
  - Publish: shield-main codeVersion 33, codeHash `e10c71d4ba56478bf64e333889225049d1812289b1f3662eb010bcc2a36ad4d2`.
  - Real run requested 20 matches but stopped after 4 by drawdown stop 80: `/tmp/agentank-runs/shield-v33-star-sample-20/challenge-runs/2026-07-07T09-00-06-195Z.json`; summaries in `/tmp/agentank-runs/shield-v33-star-sample-20/raw-replay`.
  - Result: 0 wins / 4 losses, score 1745 -> 1662 during the run, no star-win games. Aggregate stars were 4-6 against us; skill casts 3 total; shots fired 2 total. Loss shapes: 3 crashed losses and 1 runTime loss. One match led on stars 2-1 (`mat_F9uwulAdBnvIo2pUF`) but still died, so this did not validate "以吃星星赢".
  - Per-match stars: `mat_F9uwulAdBnvIo2pUF` lost crashed, stars 2-1; `mat_7iLuc0GawgMLaAkbF` lost crashed, stars 1-1; `mat_LfUiAOTWSDE8buEVc` lost runTime, stars 1-1; `mat_68eDe7lOrJl0nsU05` lost crashed, stars 0-3.
  - Review: `npm run review:batch -- /tmp/agentank-runs/shield-v33-star-sample-20/raw-replay challenger` reported 0/4 wins, avg behavior score 29.5/100, repeated `fix-no-pressure-loss` and `fix-no-value-created`. Treat this candidate as rejected, not as a protected direction.
  - Rollback: online code was immediately republished from `HEAD:active/shield-main.js` as codeVersion 34, codeHash `b8179b8c603b7f94c529c36b789f3fbf8ef7b253def11f2a2d9006ac851ffa05`.
  - Live rollback snapshot: `/tmp/agentank-runs/snapshots-shield-v34-rollback/shield-main/tank.json` confirms tank 4839 `500`, skillType boost, rankScore 1642, Master II, record 680-613-0, codeVersion 34. The local v33 route-planner diff remains uncommitted for review and must not be republished without rewriting.
- Shield-main / tank 4839 `500` local boost-route-planner candidate:
  - Code quality finding: boost decisions were still partly based on `first + dist` path summaries, while the live strategy needs route shape, turn points, and boosted landing safety. That could let a folded route spend boost even when the first boosted move would immediately leave the intended route.
  - Bounded patch: added a boost-only route trace and route planner, rejected short-turn folded boost routes, and guarded star/boost-control movement so lower star modules cannot bypass the boost planner while boosted.
  - Regressions added: no boost when a folded star route needs an immediate one-cell turn, and static coverage that boost star routes are planned with a full trace before spending boost.
  - Verification so far: focused shield-main tests passed 89/89; `npm run check` passed; `npm run test:lab` passed; `git diff --check` passed. Simulation and challenge dry-run still need to run before publish.
