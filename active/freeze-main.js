...
function onIdle(me, enemy, game) {
  ...
  function tryFreezeTempo(starInfo) {
    ...
    if (starInfo) {
      ...
      var skillRace = enemySkillIs("boost") || enemySkillIs("teleport");
      if (starInfo.myDist <= 10 && canSwingRace && enemyRace <= starInfo.myDist + 1 &&
          (skillRace || starsOf(me) < starsOf(enemy) || enemyRace <= 2)) {
        me.freeze();
        return true;
      }
    }
  ...
  function tryContestedStarLineHold(starInfo) {
    if (!game.star || !enemyTank || !starInfo) return false;
    var star = game.star;
    if (same(myPos, star)) return false;
    if (starInfo.myDist > 4) return false;
    if (starInfo.enemyRace > 3 && starInfo.enemyDist > 3) return false;
    if (px !== star[0] && py !== star[1]) return false;

    var want = dirTo(myPos, star);
    var step = add(myPos, delta(want));
    if (safeCell(step[0], step[1], true)) return false;

    var skillRace = enemySkillIs("boost") || enemySkillIs("teleport");
    if (freezeReady() && !enemyDebuffed() &&
        (skillRace || starsOf(me) < starsOf(enemy))) {
      me.freeze();
      return true;
    }

    if (bulletThreatAt(px, py, 6) || overloadThreatAt(px, py) || hiddenLaneAt(px, py)) return false;
    if (dir !== want) return turnTo(want);
    return true;
  }
  ...
  if ((!game.star || dist(px, py, ex, ey) <= 6 || pathDist(myPos, game.star) > 2) &&
      tryAttack(false)) return;
  ...
  if (game.star) {
    ...
    if (tryContestedStarLineHold(starInfo)) return;
